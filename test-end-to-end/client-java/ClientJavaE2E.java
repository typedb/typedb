    ...
    public void clientJavaE2E() {
        ...
        localhostGraknTx(tx -> {
            DefineQuery defineQuery = Graql.define(
                    Graql.type("child-bearing").sub("relationship").relates("offspring").relates("child-bearer"),
                    Graql.type("mating").sub("relationship").relates("male-partner").relates("female-partner").plays("child-bearer"),
                    Graql.type("parentship").sub("relationship").relates("parent").relates("child"),

                    Graql.type("name").sub("attribute").datatype(Query.DataType.STRING),
                    Graql.type("lion").sub("entity").has("name").plays("male-partner").plays("female-partner").plays("offspring"),

                    Graql.type("infer-parentship-from-mating-and-child-bearing").sub("rule")
                            .when(and(
