...
import ai.grakn.graql.internal.reasoner.query.*;
...
public class Reasoner {
    ...
    private boolean checkRuleApplicableToAtom(Atom parentAtom, InferenceRule child) {
        ...
        ai.grakn.graql.internal.reasoner.query.Query parent = parentAtom.getParentQuery();
    ...
    private QueryAnswers propagateHeadIdPredicates(ai.grakn.graql.internal.reasoner.query.Query atomicQuery, ai.grakn.graql.internal.reasoner.query.Query ruleHead, QueryAnswers answers){
    ...
    private QueryAnswers answerWM(AtomicQuery atomicQuery, Set<AtomicQuery> subGoals){
        ...
        if(queryAdmissible) {
            ...
            for (Rule rl : rules) {
                ...
                rule.unify(atom);
                ai.grakn.graql.internal.reasoner.query.Query ruleBody = rule.getBody();
    ...
    private QueryAnswers resolveQuery(ai.grakn.graql.internal.reasoner.query.Query query, boolean materialise) {
    ...
    public QueryAnswers resolve(MatchQuery inputQuery, boolean materialise) {
        ai.grakn.graql.internal.reasoner.query.Query query = new ReasonerMatchQuery(inputQuery, graph);
    ...
    public MatchQuery resolveToQuery(MatchQuery inputQuery, boolean materialise) {
        ai.grakn.graql.internal.reasoner.query.Query query = new ai.grakn.graql.internal.reasoner.query.Query(inputQuery, graph);
