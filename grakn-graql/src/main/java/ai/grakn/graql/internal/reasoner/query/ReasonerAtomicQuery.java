...
public class ReasonerAtomicQuery extends ReasonerQueryImpl {
    ...
    public void DBlookup() {
        QueryAnswers lookup = new QueryAnswers(getMatchQuery().admin().stream().collect(Collectors.toList()));
    ...
    private QueryAnswers insert() {
        QueryAnswers insertAnswers = new QueryAnswers(getMatchQuery().admin().stream().collect(Collectors.toList()));
    ...
    public Stream<Map<String, Concept>> resolve(boolean materialise) {
        if (!this.getAtom().isRuleResolvable()) {
            return this.getMatchQuery().admin().stream();
        } else {
