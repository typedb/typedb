...
public class DegreeDistributionMapReduce extends GraknMapReduce<Set<ConceptId>> {
    ...
    public void safeMap(final Vertex vertex, final MapEmitter<Serializable, Set<ConceptId>> emitter) {
        if (selectedTypes.isEmpty() || vertexHasSelectedTypeId(vertex, selectedTypes)) {
            emitter.emit(vertex.value((String) persistentProperties.get(DegreeVertexProgram.DEGREE)),
                    Collections.singleton(ConceptId.of(vertex.value(Schema.VertexProperty.ID.name()))));
        } else {
    ...
    @Override
    public Map<Serializable, Set<ConceptId>> generateFinalResult(
            Iterator<KeyValue<Serializable, Set<ConceptId>>> keyValues) {
