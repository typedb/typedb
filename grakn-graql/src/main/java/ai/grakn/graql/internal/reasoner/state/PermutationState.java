 ...
 *
 */
class PermutationState extends ResolutionState {

    private final Iterator<AnswerState> answerStateIterator;

    PermutationState(Answer sub, Unifier u, QueryState parent, Set<Unifier> pus) {
        super(sub, u, parent);
        this.answerStateIterator = pus.stream().map(pu -> new AnswerState(sub.unify(pu), pu, parent)).iterator();
    }
