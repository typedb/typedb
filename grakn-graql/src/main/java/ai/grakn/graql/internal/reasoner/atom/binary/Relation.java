...
public class Relation extends TypeAtom {
    ...
    public Relation(String name, String typeVariable, Map<String, Var> roleMap, IdPredicate pred, ReasonerQuery par) {
     ...
     */
    private static VarAdmin constructRelationVar(String varName, String typeVariable, List<Pair<String, Var>> rolePlayerMappings) {
        Var var;
        if (!varName.isEmpty()) var = Graql.var(varName);
        else var = Graql.var();
        rolePlayerMappings.forEach(mapping -> {
            String rp = mapping.getKey();
    ...
    private Map<RoleType, String> getRoleConceptIdMap(){
        ...
        Map<String, IdPredicate> varSubMap = getIdPredicates().stream()
                .collect(Collectors.toMap(AtomBase::getVarName, pred -> pred));
        Map<RoleType, String> roleMap = getRoleMap();
    ...
    private Map<RoleType, String> getRoleMap(){
    ...
    public Relation addType(Type type) {
        typeId = type.getId();
        String typeVariable = getValueVariable().isEmpty()? "rel-" + UUID.randomUUID().toString() : getValueVariable();
    ...
    private Set<String> getMappedRolePlayers() {
     ...
     */
    public Set<String> getUnmappedRolePlayers() {
        Set<String> unmappedVars = getRolePlayers();
    ...
    private Map<RoleType, Pair<String, Type>> computeRoleVarTypeMap() {
        ...
        Map<Var, Pair<String, Type>> roleVarMap = new HashMap<>();
        ...
        getRelationPlayers().forEach(c -> {
            ...
            if (role != null) {
                ...
                //try directly
                String typeName = role.getTypeName().orElse(null);
                RoleType roleType = typeName != null ? (RoleType) graph.getType(typeName): null;
                //try indirectly
        ...
        relationPlayersToAllocate.forEach(casting -> {
            String varName = casting.getRolePlayer().getVarName();
        ...
        List<Pair<String, Var>> rolePlayerMappings = new ArrayList<>();
        ...
        //pattern mutation!
        atomPattern = constructRelationVar(isUserDefinedName()? varName : "", getValueVariable(), rolePlayerMappings);
     ...
     */
    private Map<RoleType, String> getIndirectRoleMap(){
        GraknGraph graph =  getParentQuery().graph();
    ...
    private Map<String, String> getUnifiers(Map<RoleType, String> childMap, Map<RoleType, String> parentMap,
                                              List<String> varsToMap, List<String> varsToAllocate){
        Map<String, String> unifiers = new HashMap<>();
        List<String> allocatedVars = new ArrayList<>();
        ...
        Iterator<String> cit = varsToMap.iterator();
    ...
    private Map<String, String> getRoleTypeUnifiers(Relation parentAtom){
        Map<RoleType, String> childMap = getIndirectRoleMap();
    ...
    private Map<String, String> getRolePlayerUnifiers(Relation parentAtom){
        Map<RoleType, String> childMap = getRoleMap();
        Map<RoleType, String> parentMap = parentAtom.getRoleMap();
        return getUnifiers(
                childMap,
                parentMap,
     ...
     */
    private Map<String, IdPredicate> getVarSubMap() {
        Map<String, IdPredicate> map = new HashMap<>();
        getIdPredicates()
                .forEach( sub -> {
    ...
    @Override
    public Pair<Atom, Map<String, String>> rewrite(Atom parentAtom, ReasonerQuery parent){
