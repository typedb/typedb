...
public class GenealogyTest {
    ...
    */
    ...
    public void testSpecificPerson(){
        Concept concept = Sets.newHashSet(genealogyKB.tx().graql().infer(false).<GetQuery>parse("match $x isa person;"))
                .iterator().next()
    ...
    public void testGender() {
        ...
        QueryAnswers answers = queryAnswers(query);
        assertEquals(answers, queryAnswers(qb.<GetQuery>parse(queryString)));
        assertEquals(answers.size(), qb.<GetQuery>parse("match $x isa person;").execute().size());
    }
