...
public class CorenessTest {
    ...
    public void testKSmallerThan2_ThrowsException() {
        try (GraknTx graph = session.open(GraknTxType.READ)) {
            graph.graql().compute().coreness().minK(1).execute();
        }
    }
    ...
    public void testOnEmptyGraph_ReturnsEmptyMap() {
        try (GraknTx graph = session.open(GraknTxType.READ)) {
            Map<Long, Set<String>> result = graph.graql().compute().coreness().execute();
    ...
    public void testImplicitTypeShouldBeIncluded() {
        ...
        try (GraknTx graph = session.open(GraknTxType.READ)) {
            ...
            result = graph.graql().compute().coreness().minK(4L).execute();
            assertEquals(1, result.size());
            assertEquals(5, result.get(4L).size());
        }
    }
    ...
    public void testDisconnectedCores() {
        ...
        try (GraknTx graph = session.open(GraknTxType.READ)) {
            ...
            result = graph.graql().compute().coreness().minK(3L).execute();
            assertEquals(1, result.size());
            assertEquals(8, result.get(3L).size());
        }
    }
    ...
    public void testConcurrency() {
        ...
        Set<Map<Long, Set<String>>> result = list.parallelStream().map(i -> {
            try (GraknTx graph = session.open(GraknTxType.READ)) {
                return Graql.compute().withTx(graph).coreness().minK(3L).execute();
            }
