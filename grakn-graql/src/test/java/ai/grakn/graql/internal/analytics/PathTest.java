...
public class PathTest {
    ...
    public void whenThereIsNoPath_PathReturnsEmptyOptional() {
        addSchemaAndEntities();
        try (GraknTx graph = session.transaction(GraknTxType.READ)) {
            assertEquals(Collections.emptyList(), graph.graql().compute(PATH).from(entityId1).to(entityId5).execute().asConceptList().get());
        }
    }
    ...
    public void testShortestPath() {
        ...
        try (GraknTx graph = session.transaction(GraknTxType.READ)) {
            ...
            List<ConceptId> path = graph.graql().compute(PATH).from(entityId1).to(relationId12).execute().asConceptList().get().get(0);
            checkPath(correctPath, path);

            Collections.reverse(correctPath);
            allPaths = Graql.compute(PATH).withTx(graph).to(entityId1).from(relationId12).execute().asConceptList().get();
            ...
            allPaths = graph.graql().compute(PATH).from(entityId1).to(entityId2).execute().asConceptList().get();
            assertEquals(1, allPaths.size());
            checkPath(correctPath, allPaths.get(0));

            Collections.reverse(correctPath);
            allPaths = graph.graql().compute(PATH).to(entityId1).from(entityId2).execute().asConceptList().get();
            assertEquals(1, allPaths.size());
            checkPath(correctPath, allPaths.get(0));
            ...
            allPaths = graph.graql().compute(PATH).to(entityId3).from(entityId2).in(thing, related).execute().asConceptList().get();
            assertEquals(1, allPaths.size());
            checkPath(correctPath, allPaths.get(0));

            Collections.reverse(correctPath);
            allPaths = graph.graql().compute(PATH).in(thing, related).to(entityId2).from(entityId3).execute().asConceptList().get();
            assertEquals(1, allPaths.size());
            checkPath(correctPath, allPaths.get(0));

            correctPath = Lists.newArrayList(entityId1, relationId12, entityId2);
            allPaths = graph.graql().compute(PATH).in(thing, related).to(entityId2).from(entityId1).execute().asConceptList().get();
            assertEquals(1, allPaths.size());
            checkPath(correctPath, allPaths.get(0));

            Collections.reverse(correctPath);
            allPaths = graph.graql().compute(PATH).in(thing, related).from(entityId2).to(entityId1).execute().asConceptList().get();
            assertEquals(1, allPaths.size());
            checkPath(correctPath, allPaths.get(0));
        }
    }
    ...
    public void testShortestPathConcurrency() {
        ...
        List<List<List<ConceptId>>> result = list.parallelStream().map(i -> {
            try (GraknTx graph = session.transaction(GraknTxType.READ)) {
                return graph.graql().compute(PATH).in(thing, related).from(entityId2).to(entityId1).execute().asConceptList().get();
            }
    ...
    public void testMultipleIndependentShortestPaths() throws InvalidKBException {
        ...
        try (GraknTx graph = session.transaction(GraknTxType.READ)) {
            List<List<ConceptId>> allPaths = graph.graql().compute(PATH).from(startId).to(endId).execute().asConceptList().get();
        ...
        try (GraknTx graph = session.transaction(GraknTxType.READ)) {
            if (GraknTestUtil.usingJanus()) {
                ...
                allPaths = graph.graql().compute(PATH).in(thing, anotherThing)
                        .to(entityId1).from(entityId4).execute().asConceptList().get();
            ...
            allPaths = graph.graql().compute(PATH).from(entityId1).to(entityId4).execute().asConceptList().get();
            assertEquals(correctPaths.size(), allPaths.size());
    ...
    public void testResourceEdges() {
        ...
        try (GraknTx graph = session.transaction(GraknTxType.READ)) {
            List<List<ConceptId>> allPaths = graph.graql().compute(PATH).from(startId).to(endId)
                    .includeAttributes(true).execute().asConceptList().get();
            assertEquals(1, allPaths.size());
    ...
    public void testResourceVerticesAndEdges() {
        ...
        try (GraknTx graph = session.transaction(GraknTxType.READ)) {
            ...
            allPaths = graph.graql().compute(PATH).from(idPerson3).to(idPower3).includeAttributes(true).execute().asConceptList().get();
            assertEquals(1, allPaths.size());
            ...
            allPaths = graph.graql().compute(PATH).from(idPerson2).to(idPower1).includeAttributes(true).execute().asConceptList().get();
            assertEquals(1, allPaths.size());
            ...
            allPaths = graph.graql().compute(PATH).includeAttributes(true).from(idPower3).to(idPower1).execute().asConceptList().get();
            assertEquals(1, allPaths.size());
