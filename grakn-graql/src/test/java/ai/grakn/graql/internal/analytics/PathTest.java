...
public class PathTest {
    ...
    public void testShortestPath() {
        ...
        try (GraknTx graph = session.open(GraknTxType.READ)) {
            ...
            List<Concept> path = graph.graql().compute().path().from(entityId1).to(relationId12).execute().getPaths().get().get(0);
            checkPath(correctPath, path);

            Collections.reverse(correctPath);
            allPaths = Graql.compute().withTx(graph).path().to(entityId1).from(relationId12).execute().getPaths().get();
    ...
    public void testMultipleIndependentShortestPaths() throws InvalidKBException {
        ...
        try (GraknTx graph = session.open(GraknTxType.READ)) {
            List<List<Concept>> allPaths = graph.graql().compute().path().from(startId).to(endId).execute().getPaths().get();
    ...
    public void testResourceEdges() {
        ...
        try (GraknTx graph = session.open(GraknTxType.READ)) {
            List<List<Concept>> allPaths = graph.graql().compute()
                    .path().from(startId).includeAttributes(true).to(endId).execute().getPaths().get();
            assertEquals(1, allPaths.size());
