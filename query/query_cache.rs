...
pub struct QueryCache {
    ...
    compile_cache: Cache<TranslatedPipelineKey, ExecutablePipeline>,
...
impl QueryCache {
    pub fn new() -> Self {
        ...
        let compile_cache = CacheBuilder::new(QUERY_EXECUTABLE_CACHE_SIZE).support_invalidation_closures().build();
    ...
    pub(crate) fn may_insert_translated(
        ...
        if may_insert {
            ...
            self.translate_cache.insert(
                ...
                (cachable_translated, translated.query_context.parameters.clone()),
            );
        }
        drop(read_lock);
    }
