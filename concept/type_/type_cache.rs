...
impl TypeCache {
    ...
    fn create_relation_caches<D>(
        ...
        let relations = snapshot
            ...
            .unwrap();
        let max_relation_id = relations
            .iter()
            .map(|r| Typed::type_id(r.vertex()).as_u16()).max().unwrap();
    ...
    fn create_role_caches<D>(
        ...
        for role in roles.into_iter() {
            ...
            let cache = RoleTypeCache {
                ...
                is_root,
                annotations,
                relates: relates.iter().filter(|relates| relates.role() == role).next().unwrap().clone(),
    ...
    fn create_attribute_caches<D>(
        ...
        let attributes = snapshot
            ...
            .unwrap();
        let max_attribute_id = attributes
            .iter()
            .map(|a| Typed::type_id(a.vertex()).as_u16()).max().unwrap();
    ...
    pub(crate) fn get_entity_type_supertypes<'a>(&self, entity_type: EntityType<'a>) -> &Vec<EntityType<'static>> {
