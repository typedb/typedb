...
typedb_error!(
    pub SchemaValidationError(component = "Schema validation", prefix = "SVL") {
        ...
        CannotSetAnnotationToInterfaceBecauseItsConstraintIsNotNarrowedByItsCapabilityConstraint(30, "Cannot set annotation to interface '{interface}' because its constraint is not narrowed by its capability constraint.", interface: Label<'static>, (source: Box<ConstraintError>)),
        CannotSetAnnotationToCapabilityBecauseItsConstraintDoesNotNarrowItsInterfaceConstraint(31, "Cannot set annotation on capability '{cap}' of '{interface}' on type '{label}' because its constraint does not narrow its interface constraint.", cap: CapabilityKind, interface: Label<'static>, label: Label<'static>, (source: Box<ConstraintError>)),
        ...
        SubtypeConstraintDoesNotNarrowSupertypeConstraint(36, "Subtype constraint on '{subtype}' does not narrow supertype constraint on '{supertype}'.", subtype: Label<'static>, supertype: Label<'static>, (source: Box<ConstraintError>)),
...
//     RelatesIsAlreadySpecialisedByASupertype(RelationType<'static>, Relates<'static>),
//     CannotManageAnnotationsForSpecialisingRelates(Relates<'static>),
