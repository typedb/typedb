        ...
        $(
            pub(crate) fn $method_name(
                ...
                if let Some(cache) = &self.type_cache {
                    ...
                    Ok(MaybeOwns::Owned(TypeReader::get_label(snapshot, type_)?.ok_or(ConceptReadError::InternalMissingLabelOfType)?))
                }
            }
        )*
    }
        ...
        $(
            pub(crate) fn $method_name(
                ...
                if let Some(cache) = &self.type_cache {
                    ...
                    Ok(Arc::new(TypeReader::get_label(snapshot, type_)?.ok_or(ConceptReadError::InternalMissingLabelOfType)?))
                }
            }
        )*
    }
