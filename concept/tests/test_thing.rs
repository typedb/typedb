...
use storage::{
    ...
    snapshot::{CommittableSnapshot, ReadSnapshot, ReadableSnapshot, SchemaSnapshot, WritableSnapshot, WriteSnapshot},
};
...
fn role_player_distinct() {
    ...
    let mut snapshot: SchemaSnapshot<WALClient> = storage.clone().open_snapshot_schema();
    {
        ...
        person_type
            ...
            .unwrap();
        company_type
            .set_plays(&mut snapshot, &type_manager, &thing_manager, employer_type, StorageCounters::DISABLED)
            .unwrap();
        ...
        assert!(
            person_1
                ...
                .is_err(),
            "Expected no indices in schema transactions"
        );
...
fn role_player_duplicates_unordered() {
    ...
    let mut snapshot: SchemaSnapshot<WALClient> = storage.clone().open_snapshot_schema();
    {
        ...
        resource_type
            .set_plays(&mut snapshot, &type_manager, &thing_manager, entry_type, StorageCounters::DISABLED)
            .unwrap();
        group_type
...
fn schema_transactions_generate_role_player_indices() {
    ...
    // Schema transaction
    let (collection_type, resource_type, group_type, owner_type, entry_type) = {
        let mut snapshot: SchemaSnapshot<WALClient> = storage.clone().open_snapshot_schema();
