...
public abstract class ThingTypeImpl extends TypeImpl implements ThingType {
    ...
    @Override
    public Stream<AttributeTypeImpl> attributes(boolean onlyKey) {
        if (onlyKey && isRoot()) {
            return declaredKeys();
        } else if (onlyKey) {
    ...
    public static class Root extends ThingTypeImpl {
        ...
        @Override
        public Stream<ThingTypeImpl> sups() {
            return Stream.of(this);
        }

        @Override
        public Stream<ThingTypeImpl> subs() {
            return subs(v -> {
                switch (v.schema()) {
                    case THING_TYPE:
        ...
        @Override
        public Stream<ThingImpl> instances() {
