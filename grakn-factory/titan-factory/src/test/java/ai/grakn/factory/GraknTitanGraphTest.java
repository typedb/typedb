...
public class GraknTitanGraphTest extends TitanTestBase{
    ...
    @Before
    public void setup(){
        graknGraph = titanGraphFactory.open(TEST_BATCH_LOADING);
    }
    ...
    private void addEntity(EntityType type){
        GraknTitanGraph graph = titanGraphFactory.open(TEST_BATCH_LOADING);
        type.addEntity();
        graph.commit();
    }
    ...
    public void testCaseSensitiveKeyspaces(){
        ...
        GraknTitanGraph case1 = factory1.open(TEST_BATCH_LOADING);
        GraknTitanGraph case2 = factory2.open(TEST_BATCH_LOADING);
    ...
    public void testClearTitanGraph(){
        GraknTitanGraph graph = new TitanInternalFactory("case", Grakn.IN_MEMORY, TEST_PROPERTIES).open(false);
        graph.clear();
        expectedException.expect(GraphRuntimeException.class);
    ...
    public void testPermanentlyClosedGraph(){
        GraknTitanGraph graph = new TitanInternalFactory("test", Grakn.IN_MEMORY, TEST_PROPERTIES).open(false);
