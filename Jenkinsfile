            ...
            withEnv(['VALIDATION_DATA=' + workspace + '/generate-SNB/readwrite_neo4j--validation_set.tar.gz',
                ...
                timeout(360) {
                    stage('Run the benchmarks') {
                        sh 'mvn clean test -P janus --projects \\"grakn-test/test-integration\\" -Dtest=*Benchmark -Dmaven.repo.local=' + workspace + '/maven -Dcheckstyle.skip=true -Dfindbugs.skip=true -Dpmd.skip=true'
                    }
