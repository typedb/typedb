...
node {
    ...
    if (env.BRANCH_NAME in ['master', 'stable'] || true) {
        ...
        stage('Run the benchmarks') {
            sh "mvn clean test --batch-mode -P janus -Dtest=*Benchmark -DargLine='-Dgrakn.conf=../../conf/main/grakn.properties' -DfailIfNoTests=false -Dgrakn.test-profile=janus -Dmaven.repo.local=${workspace}/maven -Dcheckstyle.skip=true -Dfindbugs.skip=true -Dpmd.skip=true"
