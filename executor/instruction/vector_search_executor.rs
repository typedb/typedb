...
impl VectorSearchExecutor {
    pub(crate) fn new(
        ...
        // inputs may also contain the query vector variable (e.g. bound via `given`)
...
/// Resolves the query vector from either a query parameter (literal vector) or a row-bound
/// variable (e.g. supplied via a `given` stage).
