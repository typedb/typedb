...
fn traverse_index_from_unbound() {
    ...
    let var_movie_type = conjunction.constraints_mut(context).get_or_declare_variable("movie_type", None).unwrap();
    let var_casting_type = conjunction.constraints_mut(context).get_or_declare_variable("casting_type", None).unwrap();
    let var_casting_movie_type =
        conjunction.constraints_mut(context).get_or_declare_variable("casting_movie_type", None).unwrap();
    let var_casting_character_type =
        conjunction.constraints_mut(context).get_or_declare_variable("casting_character_type", None).unwrap();

    let var_movie = conjunction.constraints_mut(context).get_or_declare_variable("movie", None).unwrap();
    let var_character = conjunction.constraints_mut(context).get_or_declare_variable("character", None).unwrap();
...
fn traverse_index_from_bound() {
    ...
    let var_movie_type = conjunction.constraints_mut(context).get_or_declare_variable("movie_type", None).unwrap();
    let var_casting_type = conjunction.constraints_mut(context).get_or_declare_variable("casting_type", None).unwrap();
    let var_id_type = conjunction.constraints_mut(context).get_or_declare_variable("id_type", None).unwrap();
    let var_casting_movie_type =
        conjunction.constraints_mut(context).get_or_declare_variable("casting_movie_type", None).unwrap();
    let var_casting_actor_type =
        conjunction.constraints_mut(context).get_or_declare_variable("casting_actor_type", None).unwrap();
    ...
    conjunction.constraints_mut(context).add_isa(IsaKind::Subtype, var_movie, var_movie_type.into(), None).unwrap();
    conjunction.constraints_mut(context).add_isa(IsaKind::Subtype, var_casting, var_casting_type.into(), None).unwrap();
    conjunction.constraints_mut(context).add_isa(IsaKind::Subtype, var_id, var_id_type.into(), None).unwrap();
    conjunction.constraints_mut(context).add_label(var_movie_type, MOVIE_LABEL.clone()).unwrap();
    conjunction.constraints_mut(context).add_label(var_casting_type, CASTING_LABEL.clone()).unwrap();
    conjunction.constraints_mut(context).add_label(var_id_type, ID_LABEL.clone()).unwrap();
    conjunction.constraints_mut(context).add_label(var_casting_movie_type, CASTING_MOVIE_LABEL.clone()).unwrap();
    conjunction.constraints_mut(context).add_label(var_casting_actor_type, CASTING_ACTOR_LABEL.clone()).unwrap();
    conjunction
        .constraints_mut(context)
...
fn traverse_index_bound_role_type_filtered_correctly() {
    ...
    let var_casting_other_type =
        conjunction.constraints_mut(context).get_or_declare_variable("casting_other_type", None).unwrap();
