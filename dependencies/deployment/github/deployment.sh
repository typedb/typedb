#!/usr/bin/env bash

set -x

# configurations
grakn_core_distribution_name="grakn-core-console-$version"
github_token="ecc71a66996c225b5b3759e85b508d96b3e8f885"
github_user="lolski"
github_repository="test-ghr"
github_tag="1.0"
tmp_directory="tmp"
ghr_basename="ghr_v0.10.2_darwin_386"
ghr_zip="$ghr_base/$ghr_basename.zip"

# 0. initialise tmp directory
mkdir "$tmp_directory"

# 1. building zip
version=`cat VERSION`
distribution="$grakn_core_distribution_name.zip"
base_dir="console/dist/grakn-core-console"
cp -RL "$base_dir" "$tmp_directory/$grakn_core_distribution_name"
zip -r "$tmp_directory/$distribution" "$tmp_directory/$grakn_core_distribution_name"

# 2. create a draft release
ghr_base="external/ghr/file"
unzip "$ghr_zip" -d "$tmp_directory/"
"$tmp_directory/$ghr_basename/ghr" -t "$github_token" -u "$github_user" -r "$github_repository" -delete -draft "$github_tag" "$tmp_directory/$distribution"

# 3. cleanup tmp directory
rm -rf "$tmp_directory"