...
fn create_database(databases_path: &TempDir) -> Arc<Database<WALClient>> {
    let database_manager = DatabaseManager::new(databases_path, Arc::new(diagnostics::diagnostics_manager::DiagnosticsManager::new_test())).expect("Expected database manager");
