...
use crate::{
    migration::{
        Checksums,
        item::{MigrationMessage, encode_attribute, encode_entity, encode_relation},
    },
...
pub struct DatabaseExporter<'a> {
    ...
    opening: std::vec::IntoIter<MigrationMessage>,
...
impl<'a> DatabaseExporter<'a> {
    pub fn new(
        ...
        let header = MigrationMessage::Header {
            typedb_version: typedb_version.into(),
            original_database: original_database.into(),
        };
        Ok(Self {
            transaction,
            opening: vec![MigrationMessage::Schema(transaction.schema()?), header].into_iter(),
    ...
    pub fn next_batch(&mut self, batch_size: usize) -> Result<Option<Vec<MigrationMessage>>, DatabaseReadError> {
    ...
    pub fn next_item(&mut self) -> Result<Option<MigrationMessage>, DatabaseReadError> {
        ...
        if self.checksums_pending {
            ...
            return Ok(Some(MigrationMessage::Checksums(self.checksums.clone())));
        }
        Ok(None)
    }
}
