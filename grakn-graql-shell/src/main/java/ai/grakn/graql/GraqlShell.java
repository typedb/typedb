...
import static ai.grakn.util.REST.RemoteShell.ACTION_KEYSPACE;
...
import static ai.grakn.util.REST.RemoteShell.ACTION_QUERY_END;
...
public class GraqlShell {
    ...
    public static void runShell(String[] args, String version, String historyFilename, GraqlClient client) {
        ...
        try {
            ...
            URI uri = new URI("ws://" + uriString + REST.WebPath.REMOTE_SHELL_URI);
    ...
    private static void sendBatchRequest(String uriString, String graqlPath) throws IOException {
        ...
        URL url = new URL("http://" + uriString + REST.WebPath.IMPORT_DATA_URI);
    ...
    GraqlShell(
        ...
        try {
            ...
            sendJson(Json.object(REST.RemoteShell.ACTION, REST.RemoteShell.ACTION_KEYSPACE, REST.RemoteShell.KEYSPACE, keyspace));
    ...
    public void onMessage(String msg) {
        ...
        switch (json.at(REST.RemoteShell.ACTION).asString()) {
            case REST.RemoteShell.ACTION_QUERY:
                String result = json.at(REST.RemoteShell.QUERY_RESULT).asString();
                print(result);
                break;
            case REST.RemoteShell.ACTION_QUERY_END:
            ...
            case REST.RemoteShell.ACTION_AUTOCOMPLETE:
                ...
                break;
            case REST.RemoteShell.ACTION_ERROR:
                System.err.print(json.at(REST.RemoteShell.ERROR).asString());
                break;
        }
    }
    ...
    private void ping() {
        try {
            ...
            while (true) {
                sendJson(Json.object(REST.RemoteShell.ACTION, REST.RemoteShell.ACTION_PING));
        ...
        try {
            ...
            for (String queryChunk : splitQuery) {
                sendJson(Json.object(
                        REST.RemoteShell.ACTION, REST.RemoteShell.ACTION_QUERY,
                        REST.RemoteShell.QUERY, queryChunk
                ));
            }

            sendJson(Json.object(REST.RemoteShell.ACTION, REST.RemoteShell.ACTION_QUERY_END));
    ...
    private void commit() {
        sendJson(Json.object(REST.RemoteShell.ACTION, REST.RemoteShell.ACTION_COMMIT));
    }

    private void rollback() {
        sendJson(Json.object(REST.RemoteShell.ACTION, REST.RemoteShell.ACTION_ROLLBACK));
    }
    ...
    public void interrupt() {
        if (waitingQuery) {
            sendJson(Json.object(REST.RemoteShell.ACTION, REST.RemoteShell.ACTION_QUERY_ABORT));
            waitingQuery = false;
        } else {
            System.exit(0);
        }
    ...
    public synchronized Json getAutocompleteCandidates(
        ...
        sendJson(Json.object(
                REST.RemoteShell.ACTION, REST.RemoteShell.ACTION_AUTOCOMPLETE,
                REST.RemoteShell.QUERY, queryString,
                REST.RemoteShell.AUTOCOMPLETE_CURSOR, cursorPosition
        ));
