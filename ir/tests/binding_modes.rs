...
fn optionally_bound_in_one_branch() {
    ...
    assert_vars!(&translated_pipeline.variable_registry, optional.conjunction(), Required[], Bound["p", "name"]);
    ...
    assert_optionals!(&translated_pipeline.variable_registry, optional.conjunction(), []);
    assert_optionals!(&translated_pipeline.variable_registry, second_conjunction, []);
}

#[test]
fn optionally_assigned_is_input_to_sibling_try() {
    let function_index = {
        let query = r#"
        with fun five_or_none() -> {integer?}:
        match
            try {
                { let $x = 5; } or { let $x = 6; $x == 5; };
            };
        return { $x };
        ...
        "#;
        let parsed = typeql::parse_query(query).unwrap().into_structure().into_pipeline();
        HashMapFunctionSignatureIndex::build(
            parsed.preambles.iter().enumerate().map(|(i, preamble)| (FunctionID::Preamble(i), &preamble.function)),
        )
    };
    {
        ...
        "#;
        let parsed = typeql::parse_query(query).unwrap().into_structure();
        let translation_error = translate_pipeline(&function_index, &parsed.into_pipeline()).unwrap_err();
        assert!(match *translation_error {
            RepresentationError::UnsafeOptionalDereference { variable, .. } => {
                variable == "x"
            }
    ...
    "#;
    let parsed = typeql::parse_query(query).unwrap().into_structure();
    let translated_pipeline = translate_pipeline(&function_index, &parsed.into_pipeline()).unwrap();
    let TranslatedStage::Match { block: first_block, .. } = &translated_pipeline.translated_stages[0] else {
        unreachable!();
    };
    let first_conjunction = first_block.conjunction();
    let optional = first_conjunction.nested_patterns().first().unwrap().as_optional().unwrap();
    assert_vars!(&translated_pipeline.variable_registry, first_conjunction, Required[], Bound["x", "y"]);
    assert_vars!(&translated_pipeline.variable_registry, optional, Required["x"], Bound["y"]);
    assert_vars!(&translated_pipeline.variable_registry, optional.conjunction(), Required["x"], Bound["y"]);

    assert_optionals!(&translated_pipeline.variable_registry, first_conjunction, ["x", "y"]);
    assert_optionals!(&translated_pipeline.variable_registry, optional, ["x", "y"]);
    assert_optionals!(&translated_pipeline.variable_registry, optional.conjunction(), []);
}
