...
pub trait FromTypeQLLiteral: Sized {
    type TypeQLLiteral;
    fn from_typeql_literal(literal: &Self::TypeQLLiteral, source_span: Option<Span>) -> Result<Self, LiteralParseError>;
}

fn parse_primitive<T: FromStr>(fragment: &str, source_span: Option<Span>) -> Result<T, LiteralParseError> {
    fragment.parse::<T>().map_err(|_| LiteralParseError::FragmentParseError {
...
impl FromTypeQLLiteral for Value<'static> {
    ...
    fn from_typeql_literal(literal: &Self::TypeQLLiteral, source_span: Option<Span>) -> Result<Self, LiteralParseError> {
        ...
        match &literal.inner {
            ...
            ValueLiteral::DateTime(datetime) => Ok(Value::DateTime(NaiveDateTime::from_typeql_literal(datetime, source_span)?)),
            ...
            ValueLiteral::String(string) => Ok(Value::String(Cow::Owned(String::from_typeql_literal(string, source_span)?))),
...
impl FromTypeQLLiteral for NaiveDate {
    ...
    fn from_typeql_literal(literal: &Self::TypeQLLiteral, source_span: Option<Span>) -> Result<Self, LiteralParseError> {
        ...
        NaiveDate::from_ymd_opt(year, month, day).ok_or(LiteralParseError::InvalidDate { year, month, day, source_span })
    }
}
...
impl FromTypeQLLiteral for NaiveTime {
    ...
    fn from_typeql_literal(literal: &Self::TypeQLLiteral, source_span: Option<Span>) -> Result<Self, LiteralParseError> {
        ...
        let second = if let Some(second) = &literal.second { parse_primitive(second.as_str(), source_span)? } else { 0 };
        ...
        match literal {
            ...
            typeql::value::TimeZone::ISO(value) => Ok(TimeZone::Fixed(
                FixedOffset::from_str(value)
                    .map_err(|_| LiteralParseError::InvalidTimezoneFixedOffset { offset: value.clone(), source_span })?,
            )),
        }
    }
}
    ...
    fn from_typeql_literal(literal: &Self::TypeQLLiteral, source_span: Option<Span>) -> Result<Self, LiteralParseError> {
        ...
        match literal {
            ...
            DurationLiteral::DateAndTime(date_part, time_part) => {
                months +=
                    MONTHS_PER_YEAR * date_part.years.as_ref().map(|v| u32::from_typeql_literal(v, source_span)).transpose()?.unwrap_or(0);
                months += date_part.months.as_ref().map(|v|u32::from_typeql_literal(v, source_span)).transpose()?.unwrap_or(0);
                days += date_part.days.as_ref().map(|v| u32::from_typeql_literal(v, source_span)).transpose()?.unwrap_or(0);

                if let Some(time_part) = time_part {
                    nanos += NANOS_PER_HOUR
                        * time_part.hours.as_ref().map(|v|u64::from_typeql_literal(v, source_span)).transpose()?.unwrap_or(0);
                    nanos += NANOS_PER_MINUTE
                        * time_part.minutes.as_ref().map(|v|u64::from_typeql_literal(v, source_span)).transpose()?.unwrap_or(0);
...
impl FromTypeQLLiteral for String {
    ...
    fn from_typeql_literal(literal: &Self::TypeQLLiteral, source_span: Option<Span>) -> Result<Self, LiteralParseError> {
        literal
            .unescape()
            .map_err(|err| LiteralParseError::CannotUnescapeString { literal: literal.clone(), typedb_source: err, source_span })
    }
}
...
impl FromTypeQLLiteral for AnnotationRegex {
    ...
    fn from_typeql_literal(literal: &Self::TypeQLLiteral, source_span: Option<Span>) -> Result<AnnotationRegex, LiteralParseError> {
        Ok(AnnotationRegex::new(literal.regex.unescape_regex().map_err(|err| {
            LiteralParseError::CannotUnescapeRegexString { literal: literal.regex.clone(), source_span, typedb_source: err }
        })?))
    }
}
