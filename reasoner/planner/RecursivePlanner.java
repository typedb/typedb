...
public class RecursivePlanner extends ReasonerPlanner {
    ...
    private final OrderingCost orderingCost;
    ...
    public RecursivePlanner(TraversalEngine traversalEng, ConceptManager conceptMgr, LogicManager logicMgr) {
        ...
        this.orderingCost = new OrderingCost(this);
    }
    ...
    private void recursivelyGenerateOrderingChoices(CallMode callMode) {
        if (!orderingChoices.containsKey(callMode)) {
            ...
            for (List<Resolvable<?>> ordering : porSearch.allOrderings()) {
                ...
                OrderingSummary orderingSummary = orderingCost.createOrderingSummary(callMode, ordering);
            ...
            this.orderingChoices.put(callMode, iterate(bestOrderings.values()).map(orderingCost::createOrderingChoice).toSet());
        }
    }
    ...
    private static class OrderingCost {
        private final RecursivePlanner planner;
        private final AnswerCountEstimator answerCountEstimator;

        private OrderingCost(RecursivePlanner planner) {
            this.planner = planner;
            this.answerCountEstimator = planner.answerCountEstimator;
        }
        ...
        private OrderingSummary createOrderingSummary(CallMode callMode, List<Resolvable<?>> ordering) {
            ...
            ConjunctionNode conjunctionNode = planner.conjunctionGraph.conjunctionNode(callMode.conjunction);
        ...
        private OrderingChoice createOrderingChoice(OrderingSummary summary) {
            ConjunctionNode conjunctionNode = planner.conjunctionGraph.conjunctionNode(summary.callMode.conjunction);
        ...
        private double scaledAcyclicCost(double scalingFactor, ConjunctionNode conjunctionNode, Resolvable<?> resolvable, Set<Variable> resolvableMode) {
            ...
            if (resolvable.isRetrievable()) {
                ...
                cost += scalingFactor * retrievalCost(conjunctionNode.conjunction(), resolvable, resolvableMode);
                Set<CallMode> acyclicCalls = planner.triggeredCalls(resolvable.asConcludable(), resolvableMode, conjunctionNode.acyclicDependencies(resolvable.asConcludable()));
        ...
        private double scaledCallCost(double scalingFactor, CallMode callMode) {
            Plan plan = planner.getPlan(callMode);
