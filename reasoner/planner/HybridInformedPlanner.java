...
public class HybridInformedPlanner extends RecursivePlanner {
    ...
    protected HybridInformedPlanner(TraversalEngine traversalEng, ConceptManager conceptMgr, LogicManager logicMgr) {
        super(traversalEng, conceptMgr, logicMgr);
    }

    @Override
    Set<OrderingCoster.LocalAllCallsCosting>  recursivelyGenerateCostings(CallMode callMode) {
        HybridAStarBeamSearch ucs = new HybridAStarBeamSearch(this, callMode);
        ucs.search();
        return iterate(ucs.completedSignatures().values())
                .map(fringeElement -> fringeElement.costingBuilder.build())
                .map(singleCallCosting -> orderingCoster.createAllCallsCosting(singleCallCosting.callMode, singleCallCosting.ordering, singleCallCosting.cyclicConcludableModes))
                .toSet();
    }

    private static class HybridAStarBeamSearch {
        private final HybridInformedPlanner planner;
        ...
        public HybridAStarBeamSearch(HybridInformedPlanner planner, ReasonerPlanner.CallMode callMode) {
            ...
            this.valueIdentityResolvables = iterate(this.resolvables).filter(resolvable -> !resolvable.isNegated()).filter(r ->
                    iterate(estimateableVariables(r.variables())).flatMap(v -> iterate(v.constraints()))
                            .anyMatch(constraint ->
        ...
        public Map<Set<Pair<Concludable, Set<Variable>>>, FringeElement> completedSignatures() {
            ...
            while (!fringe.isEmpty()) {
                ...
                List<SearchNode> connectedSuccessors = iterate(successors)
                        .filter(node -> valueIdentityResolvables.contains(node.lastResolvable()) ||
                                iterate(estimateableVariables(node.lastResolvable().variables())).anyMatch(currentBoundVars::contains))
                        .toList();
        ...
        private PriorityQueue<FringeElement> beamSearch(PriorityQueue<FringeElement> fringe, Set<SearchNode> seenNodes) {
            ...
            while (!fringe.isEmpty()) {
                ...
                int maxToPoll = HybridInformedPlanner.MAX_BEAM_SIZE;
                while (maxToPoll > 0 && !fringe.isEmpty()) {
                    ...
                    List<SearchNode> connectedSuccessors = iterate(successors)
                            .filter(node -> valueIdentityResolvables.contains(node.lastResolvable()) ||
                                    iterate(estimateableVariables(node.lastResolvable().variables())).anyMatch(v -> currentBoundVars.contains(v)))
                            .toList();
        ...
        private FringeElement extendFringeTowards(FringeElement prevElement, SearchNode succNode) {
            ...
            planner.initialiseResolvableDependencies(conjunctionNode, lastResolvable, Collections.intersection(prevElement.bounds(), estimateableVariables(lastResolvable.variables())));
        ...
        private static class SearchNode {
            ...
            public SearchNode extend(Resolvable<?> res, boolean isCyclicConcludable, Set<Variable> presentBounds) {
                ...
                if (isCyclicConcludable) {
                    ...
                    nextCyclicConcludables.add(new Pair<>(res.asConcludable(), Collections.intersection(presentBounds, estimateableVariables(res.variables()))));
                } else {
