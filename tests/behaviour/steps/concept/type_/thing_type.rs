...
macro_rules! with_type {
    ($tx:ident, $kind:ident, $label:ident, $assign_to:ident, $block:block) => {
        match $kind.to_typedb() {
            ...
            Kind::Entity => {
                let $assign_to = $tx.type_manager.get_entity_type(&tx.snapshot, &snapshot, &$label.to_typedb()).unwrap().unwrap();
                $block
            },
            Kind::Relation => {
...
//     tx_as_schema! (tx, {
//         with_type!(tx, root_label, type_label, type_, {
//             type_.set_label(&tx.snapshot, &tx.type_manager, &to_label.to_typedb()).unwrap()
//         });
//     });
// }
//
...
//         match root_label.to_typedb() {
//             Kind::Attribute => {
//                 let type_ = tx.type_manager.get_attribute_type(&tx.snapshot, &type_label.to_typedb()).unwrap().unwrap();
//                 let res = type_.set_annotation(&tx.snapshot, &tx.type_manager, AttributeTypeAnnotation::Abstract(AnnotationAbstract::new()));
//                 may_error.check(&res);
//             },
//             Kind::Entity => {
//                 let type_ = tx.type_manager.get_entity_type(&tx.snapshot, &type_label.to_typedb()).unwrap().unwrap();
//                 let res = type_.set_annotation(&tx.snapshot, &tx.type_manager, EntityTypeAnnotation::Abstract(AnnotationAbstract::new()));
//                 may_error.check(&res);
//             },
//             Kind::Relation => {
//                 let type_ = tx.type_manager.get_relation_type(&tx.snapshot, &type_label.to_typedb()).unwrap().unwrap();
//                 let res = type_.set_annotation(&tx.snapshot, &tx.type_manager, RelationTypeAnnotation::Abstract(AnnotationAbstract::new()));
//                 may_error.check(&res);
//             },
//             Kind::Role => {
//                 let type_ = tx.type_manager.get_role_type(&tx.snapshot, &type_label.to_typedb()).unwrap().unwrap();
//                 let res = type_.set_annotation(&tx.snapshot, &tx.type_manager, RoleTypeAnnotation::Abstract(AnnotationAbstract::new()));
//                 may_error.check(&res);
//             },
//         };
//     });
