...
public class Query {
    ...
    public Pattern.Disjunction<Pattern.Conjunction<Var.Admin>> getDNF(){
        return matchQuery.admin().getPattern().getDisjunctiveNormalForm();}

    public Set<AtomConjunction> getAtomConjunctions() {
        Set<AtomConjunction> conj = new HashSet<>();
        getDNF().getPatterns().forEach(c -> conj.add(new AtomConjunction(c)));
        return conj;
    }
    public Set<AtomConjunction> getExpandedAtomConjunctions() {
        Set<AtomConjunction> conj = new HashSet<>();
        getExpandedDNF().getPatterns().forEach(c -> conj.add(new AtomConjunction(c)));
        return conj;
    }
    public Pattern.Disjunction<Pattern.Conjunction<Var.Admin>> getExpandedDNF() {
