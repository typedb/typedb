...
public class PredicateConstraint extends ValueConstraint implements AlphaEquivalent<PredicateConstraint> {

    final Predicate<?> predicate;
    private final int hash;

    public PredicateConstraint(ValueVariable owner, Predicate<?> predicate) {
    ...
    static PredicateConstraint of(ValueVariable owner, PredicateConstraint toClone, VariableCloner cloner) {
    ...
    public void addTo(GraphTraversal.Thing traversal) {
        if (predicate.isConstant()) {
            ...
            if (asConstant.valueType().equals(Encoding.ValueType.BOOLEAN)) {
                traversal.predicateValue(owner.id(), predicate.predicate().asEquality(), asConstant.asBoolean().value());
            } else if (asConstant.valueType().equals(Encoding.ValueType.LONG)) {
                traversal.predicateValue(owner.id(), predicate.predicate().asEquality(), asConstant.asLong().value());
            } else if (asConstant.valueType().equals(Encoding.ValueType.DOUBLE)) {
                traversal.predicateValue(owner.id(), predicate.predicate().asEquality(), asConstant.asDouble().value());
            } else if (asConstant.valueType().equals(Encoding.ValueType.DATETIME)) {
                traversal.predicateValue(owner.id(), predicate.predicate().asEquality(), asConstant.asDateTime().value());
            } else if (asConstant.valueType().equals(Encoding.ValueType.STRING) && asConstant.predicate().isEquality()) {
                traversal.predicateValue(owner.id(), predicate.predicate().asEquality(), asConstant.asString().value());
            } else if (asConstant.valueType().equals(Encoding.ValueType.STRING) && asConstant.predicate().isSubString()) {
                traversal.predicateValue(owner.id(), predicate.predicate().asSubString(), asConstant.asString().value());
            } else throw TypeDBException.of(ILLEGAL_STATE);
        } else if (predicate.isThingVar()) {
            traversal.predicateValueThing(owner.id(), predicate.predicate().asEquality(), predicate.asThingVar().value().id());
        } else if (predicate.isValueVar()) {
            traversal.predicateValueValue(owner.id(), predicate.predicate().asEquality(), predicate.asValueVar().value().id());
        } else throw TypeDBException.of(ILLEGAL_STATE);
    }
