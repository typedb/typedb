...
use super::primitive_encoding::{decode_i32, decode_u32, encode_i32, encode_u32};
...
impl TimeZone {
    pub(crate) fn encode(self) -> [u8; 4] {
        match self {
            TimeZone::IANA(tz) => encode_u32(encode_tz(tz)),
            TimeZone::Fixed(fixed) => {
                // from UTC-23:59:59 to UTC+23:59:59 <=> -86399..=86399
                encode_i32(fixed.local_minus_utc())
            }
        }
    }

    pub(crate) fn decode(encoded: [u8; 4]) -> Self {
        let as_u32 = decode_u32(encoded);
        if as_u32 < NUM_TZS {
            Self::IANA(decode_tz(as_u32))
        } else {
            let secs = decode_i32(encoded);
            Self::Fixed(FixedOffset::east_opt(secs).unwrap())
        }
    }
}
...
macro_rules! tz_to_number {
    ($($id:literal => $tz:path),+ $(,)?) => {
        ...
        const fn encode_tz(tz: Tz) -> u32 {
            #[deny(unreachable_patterns)]
            match tz { $($tz => $id,)+ }
        }
        const fn decode_tz(id: u32) -> Tz {
            assert!(id  < NUM_TZS);
            #[deny(unreachable_patterns)]
            match id {
                $($id => $tz,)+
