...
use encoding::{AsBytes, create_keyspaces};
...
fn generate_string_attribute_vertex() {
    ...
    let snapshot = Snapshot::Write(storage.open_snapshot_write());
    ...
    // 1: vertex for short string that is stored inline
    {
        ...
        let vertex = thing_vertex_generator.create_attribute_string(type_id, short_string_bytes.clone_as_ref(), &snapshot);
        let vertex_id = StringAttributeID::new(vertex.attribute_id().unwrap_bytes_16());
    ...
    // 2: vertex for long string that does not exist beforehand with default hasher
    {
        ...
        let vertex_id = StringAttributeID::new(vertex.attribute_id().unwrap_bytes_16());
        assert!(!vertex_id.is_inline());
        assert_eq!(vertex_id.get_hash_prefix(), string_bytes.bytes().bytes()[StringAttributeID::ENCODING_STRING_PREFIX_RANGE]);
        assert_eq!(vertex_id.get_hash_hash(), seahash::hash(string_bytes.bytes().bytes()).to_be_bytes()[0..StringAttributeID::ENCODING_STRING_HASH_LENGTH]);
...

    {
        ...
        assert!(!vertex_id.is_inline());
        assert_eq!(vertex_id.get_hash_prefix(), string_bytes.bytes().bytes()[StringAttributeID::ENCODING_STRING_PREFIX_RANGE]);
        assert_eq!(vertex_id.get_hash_hash(), CONSTANT_HASH.to_be_bytes()[0..StringAttributeID::ENCODING_STRING_HASH_LENGTH]);
        assert_eq!(vertex_id.get_hash_disambiguator(), 0u8);
        ...
        let collide_vertex = thing_vertex_generator.create_attribute_string(type_id, string_collide_bytes.clone_as_ref(), &snapshot);

        let collide_id = StringAttributeID::new(collide_vertex.attribute_id().unwrap_bytes_16());
        assert!(!collide_id.is_inline());
        assert_eq!(collide_id.get_hash_prefix(), string_collide_bytes.bytes().bytes()[StringAttributeID::ENCODING_STRING_PREFIX_RANGE]);
        assert_eq!(collide_id.get_hash_hash(), CONSTANT_HASH.to_be_bytes()[0..StringAttributeID::ENCODING_STRING_HASH_LENGTH]);
