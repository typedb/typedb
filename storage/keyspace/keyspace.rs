...
impl Into<KVStoreID> for KeyspaceId {
    fn into(self) -> KVStoreID {
        self.0 as KVStoreID
    }
}
...
#[derive(Debug)]
pub struct Keyspaces<KV> {
    keyspaces: Vec<KV>,
    index: [Option<KeyspaceId>; KEYSPACE_MAXIMUM_COUNT],
}

impl<KV: KVStore> Keyspaces<KV> {
    pub(crate) fn new() -> Self {
        Self { keyspaces: Vec::new(), index: std::array::from_fn(|_| None) }
    }

    pub(crate) fn open<KS: KeyspaceSet>(storage_dir: impl AsRef<Path>) -> Result<Self, KeyspacesError> {
        let path = storage_dir.as_ref();

        let shared_resources = KV::create_shared_resources();
    ...
    fn validate_new_keyspace(&self, keyspace_id: impl KeyspaceSet) -> Result<(), KeyspacesError> {
    ...
    pub(crate) fn get(&self, keyspace_id: KeyspaceId) -> &KV {
