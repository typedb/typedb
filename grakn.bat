...
if exist .\services\lib\core.console-*.jar (
  ...
  java -cp "%G_CP%" -Dgrakn.dir="%GRAKN_HOME%" -Dgrakn.conf="%GRAKN_HOME%/%GRAKN_CONFIG%" -Dengine.javaopts="%ENGINE_JAVAOPTS%" grakn.core.console.Graql %*
  goto exit
) else (
...
if exist .\services\lib\core.server-*.jar (
   ...
   java -cp "%G_CP%" -Dgrakn.dir="%GRAKN_HOME%" -Dgrakn.conf="%GRAKN_HOME%/%GRAKN_CONFIG%" -Dengine.javaopts="%ENGINE_JAVAOPTS%" grakn.core.server.daemon.GraknDaemon %*
   goto exit
) else (
