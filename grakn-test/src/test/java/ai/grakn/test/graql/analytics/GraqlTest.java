...
public class GraqlTest {
    ...
    public void testStatisticsMethods() throws InvalidGraphException {
        try (GraknGraph graph = factory.open(GraknTxType.WRITE)) {
            ...
            Role resourceOwner = graph.putRoleType(Schema.ImplicitType.HAS_OWNER.getLabel(resourceTypeId));
            Role resourceValue = graph.putRoleType(Schema.ImplicitType.HAS_VALUE.getLabel(resourceTypeId));
