...
public class TestSamplesImport extends TestOwlGraknBase {
    ...
    public void testShakespeareOntology()   {       
        ...
        try {
            ...
            Assert.assertTrue(!ReasonerUtils.getRules(graph).isEmpty());
        }
        catch (Throwable t) {
            t.printStackTrace(System.err);
            Assert.fail(t.toString());
    ...
    public void test1Ontology() {       
        ...
        try {
            ...
            RoleType subjectRole = graph.getType(migrator.namer().subjectRole(Label.of("op-related")));
            RoleType objectRole = graph.getType(migrator.namer().objectRole(Label.of("op-related")));
            ...
            RoleType catsubjectRole = graph.getType(migrator.namer().subjectRole(Label.of("op-hasCategory")));
            RoleType catobjectRole = graph.getType(migrator.namer().objectRole(Label.of("op-hasCategory")));
