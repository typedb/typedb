...
public class BootupITConstants {
    ...
    public static void assertGraknRunning() {
        assertThat("assertKgmsRunning() failed because /tmp/grakn-engine.pid is not found", Paths.get("/tmp/grakn-engine.pid").toFile().exists(), equalTo(true));
        assertThat("assertKgmsRunning() failed because /tmp/grakn-queue.pid is not found", Paths.get("/tmp/grakn-queue.pid").toFile().exists(), equalTo(true));
        assertThat("assertKgmsRunning() failed because /tmp/grakn-storage.pid is not found", Paths.get("/tmp/grakn-storage.pid").toFile().exists(), equalTo(true));
    }
