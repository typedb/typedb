...
public class BootupITConstants {
    ...
    public static void assertGraknRunning() {
        assertThat(Paths.get("/tmp/grakn-engine.pid").toFile().exists(), equalTo(true));
        assertThat(Paths.get("/tmp/grakn-queue.pid").toFile().exists(), equalTo(true));
        assertThat(Paths.get("/tmp/grakn-storage.pid").toFile().exists(), equalTo(true));
    }

    public static void assertGraknStopped() {
        assertThat(Paths.get("/tmp/grakn-engine.pid").toFile().exists(), equalTo(false));
        assertThat(Paths.get("/tmp/grakn-queue.pid").toFile().exists(), equalTo(false));
        assertThat(Paths.get("/tmp/grakn-storage.pid").toFile().exists(), equalTo(false));
    }
