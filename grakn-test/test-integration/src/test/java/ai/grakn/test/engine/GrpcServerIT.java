...
public class GrpcServerIT {
    ...
    public void whenExecutingComputeQueryies_ResultsAreCorrect() {
        ...
        try (GraknTx tx = localSession.open(GraknTxType.WRITE)) {
            ...
            EntityType pet = tx.putEntityType("pet").plays(role1);
            EntityType human = tx.putEntityType("human").plays(role2);
            RelationshipType friend = tx.putRelationshipType("friend").relates(role1).relates(role2);
            AttributeType<Long> age = tx.putAttributeType("age", DataType.LONG);
            human.attribute(age);

            Entity coco = pet.addEntity();
            Entity mike = human.addEntity();
            Relationship cocoAndMike = friend.addRelationship().addRolePlayer(role1, coco).addRolePlayer(role2, mike);
