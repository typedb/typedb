        ...
        if kinds.contains(&Kind::Relation) || kinds.contains(&Kind::Entity) {
            if kinds.contains(&Kind::Attribute) {
                return Err(Box::new(WriteCompilationError::InsertVariableAmbiguousAttributeOrObject {
                    variable: safe_variable_name(variable_registry, thing),
                    source_span: isa.source_span(),
                }));
            }
...
fn safe_variable_name(variable_registry: &VariableRegistry, thing: Variable) -> String {
    variable_registry
        .get_variable_name(thing)
        .cloned()
        .unwrap_or_else(|| VariableRegistry::UNNAMED_VARIABLE_DISPLAY_NAME.to_owned())
}
        ...
        .map(|links| {
            ...
            if let Some(input_position) = input_variables.get(&role_type) {
                ...
                if let Ok(type_) = annotations.iter().exactly_one() {
                    ...
                    let player_variable = safe_variable_name(variable_registry, links.player().as_variable().unwrap());
