...
- **Partition data into Keyspaces and configure RocksDB**
  ...
  We split the data in TypeDB into 5 RocksDB database (internally known as Keyspaces):
  ...
  We also try to squeeze a bit of performance by enable Link-Time optimisation and producing more optimal output binaries using the following cargo release profile:
  ...
  **Warnings**
  1)  We configure each database to have a default **1GB block cache for the time being**.
  2) We have **not** seen the bloom filter actually produce any hits yet (according to RocksDB statistics), and need to investigate further
  3) As noted in https://github.com/typedb/typedb/pull/7262, if we get bloom filters to activate, we will also have to careful to _deactivate_ them when the scan range escapes a single constant bloom-filtered prefix.
...
- **Restore and update greedy traversal planner**
  ...
  * PostHog (will be done in a near future).
  ...
  Example monitored JSON diagnostics (the reported diagnostics are in a similar JSON format, but with a couple of changed fields) (simplified):
  ```
  {
    "version": 1,
    "deploymentID": "IXP50DFI2J7RTQNA",
    "serverID": "IXP50DFI2J7RTQNA",
    "distribution": "TypeDB CE",
    "timestamp": "2024-12-16T18:22:48.335",
    "server": {
      ...
      "memoryUsedInBytes": 38149668864,
      "memoryAvailableInBytes": 30569807872,
      "diskUsedInBytes": 426276663032,
      "diskAvailableInBytes": 1568941502728
    },
    "load": [
      {
        "database": "95615005977010737",
        "schema": {
          "typeCount": 8
        },
        "data": {
  ...
  Example of monitored Prometheus diagnostics (simplified, some `actions` from the previous example can be missed):
  ```
  # distribution: TypeDB CE
  # version: 3.0.0-alpha-10
  # os: Darwin arm64 14.2.1
  
  # TYPE server_resources_count gauge
  server_resources_count{kind="memoryUsedInBytes"} 38218219520
  server_resources_count{kind="memoryAvailableInBytes"} 30501257216
  server_resources_count{kind="diskUsedInBytes"} 426276581112
  server_resources_count{kind="diskAvailableInBytes"} 1568941584648
  
  # TYPE typedb_schema_data_count gauge
  typedb_schema_data_count{database="95615005977010737", kind="typeCount"} 8
  typedb_schema_data_count{database="95615005977010737", kind="entityCount"} 2
  typedb_schema_data_count{database="95615005977010737", kind="relationCount"} 1
  typedb_schema_data_count{database="95615005977010737", kind="attributeCount"} 3
  typedb_schema_data_count{database="95615005977010737", kind="hasCount"} 3
  typedb_schema_data_count{database="95615005977010737", kind="roleCount"} 2
  typedb_schema_data_count{database="95615005977010737", kind="storageInBytes"} 8107
  typedb_schema_data_count{database="95615005977010737", kind="storageKeyCount"} 71
  
  # TYPE typedb_attempted_requests_total counter
  typedb_attempted_requests_total{kind="CONNECTION_OPEN"} 276
  typedb_attempted_requests_total{kind="AUTHENTICATE"} 1094
  typedb_attempted_requests_total{kind="USERS_ALL"} 88
  typedb_attempted_requests_total{kind="DATABASES_ALL"} 295
  typedb_attempted_requests_total{database="95615005977010737", kind="TRANSACTION_QUERY"} 274
  typedb_attempted_requests_total{database="95615005977010737", kind="DATABASES_CREATE"} 88
  typedb_attempted_requests_total{database="95615005977010737", kind="TRANSACTION_OPEN"} 153
  typedb_attempted_requests_total{database="95615005977010737", kind="TRANSACTION_COMMIT"} 68
  typedb_attempted_requests_total{database="95615005977010737", kind="DATABASES_DELETE"} 89
  
  # TYPE typedb_successful_requests_total counter
  typedb_successful_requests_total{kind="CONNECTION_OPEN"} 276
  typedb_successful_requests_total{kind="AUTHENTICATE"} 1094
  typedb_successful_requests_total{database="95615005977010737", kind="TRANSACTION_QUERY"} 270
  typedb_successful_requests_total{database="95615005977010737", kind="DATABASES_CREATE"} 88
  typedb_successful_requests_total{database="95615005977010737", kind="TRANSACTION_OPEN"} 153
  typedb_successful_requests_total{database="95615005977010737", kind="TRANSACTION_COMMIT"} 66
  typedb_successful_requests_total{database="95615005977010737", kind="DATABASES_DELETE"} 89
  
  # TYPE typedb_error_total counter
  typedb_error_total{database="3244421341483603138", code="DBC1"} 1 
  typedb_error_total{database="95615005977010737", code="TSV2"} 1 
  typedb_error_total{database="95615005977010737", code="SYR1"} 3
  ``` 
...
- **Type inference and pipeline fixes**
  Implements various type inference and pipeline fixes uncovered in manual QA
...
- **Async authentication layer**
  
  We make authentication layer asynchronous to avoid blocking the async runtime.
...
- **Simplify bytes and vertex representation**
  
  We now decode the storage keys eagerly, rather than storing and passing around a copy-on-write `Bytes` enum.
  
  Vertices and edges in encoding are now copyable POD wherever possible. This greatly simplifies downstream handling of lifetimes and reduces our reliance on lending iterators.
...
- **Clean up dead code & some warnings**
  Cleans up dead code & some warnings
  
  
- **Refactor compiler package modules by query type**
  Refactors compiler package modules to be organised by query type.
...
- **Spec polishing**
  ...
  This PR is to (continue to) keep track of recent changes in the specification of grammar and semantics of TypeDB + TypeQL.
...
- **Green match BDD**
  ...
  We stabilise match behaviour and add match BDD feature test to CI.
...
- **Refactor executor package in preparation for functions**
  Refactors executor package in preparation for functions
...
- **Update connection tests for 3.0**
  We update BDD steps for connection to satisfy the [updated](https://github.com/typedb/typedb-behaviour/pull/301) BDDs for `connection/database` and `connection/transaction`. 
...
- **Server listens to 0.0.0.0 instead of 127.0.0.1**
  
- **Extract parameters, remove intermediate variables for explicit types in IR, add comparison checks**

- **Bug fixes, error messages, and protocol changes to support Rust driver**
  
  1. implement lots of useful new error messages for data and commit validation
  2. implement refined protocol that reduces number of connection open round trips by 1
  3. fix various server bugs found during QA
...
- **Refactoring pipelines**
  ...
  We simplify the traits and data structures around Pipeline execution. We also optimise their execution by avoiding extra allocations and Vec<Vec<...>> constructs.
  
  We also rename `ImmutableRow` to `MaybeOwnedRow`, which allows getting a mutable reference to the Row whenever it is actually owned. `Row` remains purely mutable reference to set of row data owned elsewhere.
...
- **Rearchitect packages on the read query path**
  
  We rearchitect the `//ir` and `//traversal` packages into several different packges, to allow placing expressions and their compilation and evaluation into a better cohesive achitecture.
  
  We now have
  ```
  //executor     expression execution and pattern execution
  //compiler     which contains expression compilation, type inference, planning, and optimisation passes (future work)
  //ir      which contains only the server-side representation and translation of the TypeQL into canonical form
  ```
  
  We also have `//executor -> //compiler -> //ir`, so we each layer can re-use data structures from the one below, and encompass logic for building the more granular representation from the less granular one in the more granular package. Note this is the inverted direction of package dependencies we have normally, where we like the data flow to match the package dependency direction!
