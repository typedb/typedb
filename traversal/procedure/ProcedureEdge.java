            ...
            FunctionalIterator<? extends Vertex<?, ?>> forwardBranchToRole(GraphManager graphMgr, Vertex<?, ?> fromVertex,
                ...
                if (!toTypes.isEmpty()) {
                    iter = iterate(toTypes).map(l -> graphMgr.schema().getType(l)).noNulls()
                            .flatMap(t -> relation.outs().edge(encoding, PrefixIID.of(VERTEX_ROLE), t.iid()).to(true));
                } else {
                    iter = relation.outs().edge(encoding).to(true);
                }
                return iter;
            }
                ...
                static class Forward extends Has {
                    ...
                    public FunctionalIterator<? extends Vertex<?, ?>> branch(
                        ...
                        if (to.props().hasIID()) {
                            ...
                            else att = graphMgr.data().get(iid.asAttribute(), true);
                            ...
                            if (eq != null) {
                                ...
                                iter = iterate(to.props().types()).map(l -> graphMgr.schema().getType(l)).noNulls()
                                        .flatMap(t -> owner.outs().edge(HAS, PrefixIID.of(VERTEX_ATTRIBUTE), t.iid()).to(true))
                                        .map(ThingVertex::asAttribute);
                            }
                        } else {
                            iter = owner.outs().edge(HAS).to(true).map(ThingVertex::asAttribute);
                        }
                ...
                static class Backward extends Has {
                    ...
                    public FunctionalIterator<? extends Vertex<?, ?>> branch(
                        ...
                        if (to.props().hasIID()) {
                            ...
                            iter = iterate(to.props().types()).map(l -> graphMgr.schema().getType(l)).noNulls()
                                    .flatMap(t -> att.ins().edge(HAS, PrefixIID.of(t.encoding().instance()), t.iid()).from(true));
                        } else {
                            iter = att.ins().edge(HAS).from(true);
                        }
                ...
                static class Backward extends Playing {
                    ...
                    public FunctionalIterator<? extends Vertex<?, ?>> branch(
                        ...
                        if (to.props().hasIID()) {
                            ...
                            iter = iterate(toTypes).map(l -> graphMgr.schema().getType(l)).noNulls()
                                    .flatMap(t -> role.ins().edge(PLAYING, PrefixIID.of(t.encoding().instance()), t.iid()).from(true));
                        } else {
                            iter = role.ins().edge(PLAYING).from(true);
                        }
                ...
                static class Backward extends Relating {
                    ...
                    public FunctionalIterator<? extends Vertex<?, ?>> branch(
                        ...
                        if (to.props().hasIID()) {
                            ...
                            iter = iterate(toTypes).map(l -> graphMgr.schema().getType(l)).noNulls()
                                    .flatMap(t -> role.ins().edge(RELATING, PrefixIID.of(RELATION), t.iid()).from(true));
                        } else {
                            iter = role.ins().edge(RELATING).from(true);
                        }
                        return iter;
                    }
                    ...
                    public boolean isClosure(GraphManager graphMgr, Vertex<?, ?> fromVertex, Vertex<?, ?> toVertex,
                        ...
                        if (!roleTypes.isEmpty()) {
                            validEdge = iterate(resolvedRoleTypes(graphMgr.schema())).flatMap(
                                    rt -> rel.outs().edge(ROLEPLAYER, rt.iid(), player.iid().prefix(), player.iid().type()).get()
                                            .filter(e -> e.to(true).equals(player) && !scoped.contains(e.optimised().get())))
                                    .first();
                        } else {
                            validEdge = rel.outs().edge(ROLEPLAYER).get().filter(
                                    e -> e.to(true).equals(player) && !scoped.contains(e.optimised().get())
                            ).first();
                        }
                            ...
                            if (to.props().hasIID()) {
                                ...
                                iter = resolveRoleTypesIter.flatMap(
                                        rt -> player.ins().edge(ROLEPLAYER, rt.iid(), relation.iid().prefix(), relation.iid().type())
                                                .get().filter(r -> r.from(true).equals(relation)));
                            } else if (!to.props().types().isEmpty()) {
                                filteredTypes = true;
                                iter = resolveRoleTypesIter.flatMap(
                                        rt -> iterate(to.props().types()).map(l -> graphMgr.schema().getType(l)).noNulls()
                        ...
                        if (!roleTypes.isEmpty()) {
                            validEdge = iterate(resolvedRoleTypes(graphMgr.schema())).flatMap(
                                    rt -> player.ins().edge(ROLEPLAYER, rt.iid(), rel.iid().prefix(), rel.iid().type()).get()
                                            .filter(e -> e.from(true).equals(rel) && !scoped.contains(e.optimised().get())))
                                    .first();
                        } else {
                            validEdge = player.ins().edge(ROLEPLAYER).get().filter(
                                    e -> e.from(true).equals(rel) && !scoped.contains(e.optimised().get())
                            ).first();
                        }
                        validEdge.ifPresent(e -> scoped.push(e.optimised().get(), order()));
                        return validEdge.isPresent();
