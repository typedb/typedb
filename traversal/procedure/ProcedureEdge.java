...
abstract class ProcedureEdge<VERTEX_FROM extends ProcedureVertex<?>, VERTEX_TO extends ProcedureVertex<?>>
    ...
    public static ProcedureEdge<?, ?> of(ProcedureVertex<?> from, ProcedureVertex<?> to, PlannerEdge.Directional<?, ?> plannerEdge) {
        int ord = plannerEdge.orderNumber();
        Encoding.Direction.Edge dir = plannerEdge.direction();
        if (plannerEdge.isEqual()) {
            return new Equal(from, to, ord, dir);
        } else if (plannerEdge.isPredicate()) {
            return new Predicate(from.asThing(), to.asThing(), ord, dir, plannerEdge.asPredicate().predicate());
    ...
    static class Native<VERTEX_NATIVE_FROM extends ProcedureVertex<?>, VERTEX_NATIVE_TO extends ProcedureVertex<?>>
        ...
        static Native<?, ?> of(ProcedureVertex<?> from, ProcedureVertex<?> to, PlannerEdge.Native.Directional<?, ?> edge) {
            if (edge.isIsa()) {
                int ord = edge.orderNumber();
                boolean trns = edge.asIsa().isTransitive();
                if (edge.direction().isForward()) return new Isa.Forward(from.asThing(), to.asType(), ord, trns);
                else return new Isa.Backward(from.asType(), to.asThing(), ord, trns);
            } else if (edge.isType()) {
        ...
        static class Isa<VERTEX_ISA_FROM extends ProcedureVertex<?>, VERTEX_ISA_TO extends ProcedureVertex<?>>
        ...
        static class Type extends Native<ProcedureVertex.Type, ProcedureVertex.Type> {
        ...
        static class Thing extends Native<ProcedureVertex.Thing, ProcedureVertex.Thing> {
