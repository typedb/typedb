...
import grakn.core.kb.reasoner.cache.IndexedAnswerSet;
...
import grakn.core.kb.reasoner.utils.TarjanSCC;
...
public class RuleUtils {
    ...
    final private static Function<InferenceRule, Stream<Pair<Type, Type>>> persistedRuleToTypePair = rule ->
            rule.getBody()
                    .getAtoms(grakn.core.kb.reasoner.atom.Atom.class)
    ...
    public static Set<InferenceRule> getDependentRules(ReasonerQueryImpl query){
        ...
        Set<Equivalence.Wrapper<grakn.core.kb.reasoner.atom.Atom>> visitedAtoms = new HashSet<>();
        Stack<Equivalence.Wrapper<grakn.core.kb.reasoner.atom.Atom>> atoms = new Stack<>();
        query.selectAtoms().map(equivalence::wrap).forEach(atoms::push);
        while(!atoms.isEmpty()) {
            Equivalence.Wrapper<grakn.core.kb.reasoner.atom.Atom> wrappedAtom = atoms.pop();
             grakn.core.kb.reasoner.atom.Atom atom = wrappedAtom.get();
