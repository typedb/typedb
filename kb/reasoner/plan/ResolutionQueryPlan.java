...
package grakn.core.kb.reasoner.plan;
...
public class ResolutionQueryPlan {
    ...
    private static ImmutableList<ReasonerQueryImpl> queryPlan(ReasonerQueryImpl query){
        ...
        ImmutableList<grakn.core.kb.reasoner.atom.Atom> plan = resolutionPlan.plan();
        Transaction tx = query.tx();
        LinkedList<grakn.core.kb.reasoner.atom.Atom> atoms = new LinkedList<>(plan);
        List<ReasonerQueryImpl> queries = new LinkedList<>();

        List<grakn.core.kb.reasoner.atom.Atom> nonResolvableAtoms = new ArrayList<>();
        while (!atoms.isEmpty()) {
            grakn.core.kb.reasoner.atom.Atom top = atoms.remove();
