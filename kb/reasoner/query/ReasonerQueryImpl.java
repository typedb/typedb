...
import grakn.core.kb.reasoner.atom.binary.IsaAtomBase;
import grakn.core.kb.reasoner.atom.binary.RelationAtom;
import grakn.core.kb.reasoner.atom.predicate.IdPredicate;
import grakn.core.kb.reasoner.atom.predicate.VariablePredicate;
import grakn.core.kb.reasoner.cache.Index;
import grakn.core.kb.reasoner.explanation.JoinExplanation;
import grakn.core.kb.reasoner.explanation.LookupExplanation;
import grakn.core.kb.reasoner.plan.GraqlTraversalPlanner;
...
import grakn.core.kb.reasoner.state.AnswerState;
import grakn.core.kb.reasoner.state.ConjunctiveState;
import grakn.core.kb.reasoner.state.CumulativeState;
import grakn.core.kb.reasoner.state.VariableComparisonState;
...
public class ReasonerQueryImpl implements ResolvableQuery {
    ...
    ReasonerQueryImpl(List<grakn.core.kb.reasoner.atom.Atom> atoms, Transaction tx){
        this.tx = tx;
        this.atomSet =  ImmutableSet.<Atomic>builder()
                .addAll(atoms.stream()
    ...
    public boolean isRuleResolvable() {
        return selectAtoms().anyMatch(grakn.core.kb.reasoner.atom.Atom::isRuleResolvable);
    }
    ...
    public boolean isBoundlesslyDisconnected(){
        return !isAtomic()
                ...
                .anyMatch(grakn.core.kb.reasoner.atom.Atom::isDisconnected);
    }

    /**
     * @return true if the query requires direct schema lookups
     */
    public boolean requiresSchema(){ return selectAtoms().anyMatch(grakn.core.kb.reasoner.atom.Atom::requiresSchema);}
     ...
     */
    @Override
    public Stream<grakn.core.kb.reasoner.atom.Atom> selectAtoms() {
        return getAtoms(grakn.core.kb.reasoner.atom.Atom.class).filter(Atomic::isSelectable);
    }

    @Override
    public boolean requiresDecomposition(){
        return this.selectAtoms().anyMatch(grakn.core.kb.reasoner.atom.Atom::requiresDecomposition);
    }
    ...
    public boolean isCacheComplete(){
        ...
        for (ReasonerAtomicQuery query : queries) {
            ...
            Graql.and(
                    ...
                    query.getAtoms(grakn.core.kb.reasoner.atom.Atom.class).collect(Collectors.toList()),
                    Sets.union(
    ...
    public boolean requiresReiteration() {
        ...
        return RuleUtils.subGraphIsCyclical(dependentRules, tx())
                ...
                || selectAtoms().filter(grakn.core.kb.reasoner.atom.Atom::isDisconnected).filter(grakn.core.kb.reasoner.atom.Atom::isRuleResolvable).count() > 1;
    }

    @Override
    public grakn.core.kb.reasoner.state.ResolutionState resolutionState(ConceptMap sub, Unifier u, grakn.core.kb.reasoner.state.AnswerPropagatorState parent, Set<ReasonerAtomicQuery> subGoals){
     ...
     */
    public Stream<grakn.core.kb.reasoner.state.ResolutionState> expandedStates(ConceptMap sub, Unifier u, grakn.core.kb.reasoner.state.AnswerPropagatorState parent, Set<ReasonerAtomicQuery> subGoals){
    ...
    @Override
    public Iterator<grakn.core.kb.reasoner.state.ResolutionState> innerStateIterator(grakn.core.kb.reasoner.state.AnswerPropagatorState parent, Set<ReasonerAtomicQuery> subGoals){
        Iterator<AnswerState> dbIterator;
        Iterator<grakn.core.kb.reasoner.state.AnswerPropagatorState> subGoalIterator;
