...
//
    ...
    public void mainTest() {
        System.out.println("Checking if there's any dangling Redis process...");

        Integer redis_pid=null; //TODO: = "`ps -ef | grep 'redis-server' | grep -v grep | awk '{ print $2}'`";
        if(redis_pid==null) {
            System.out.println("Force killing Redis (pid=$redis_pid)");
            // TODO: kill redis
        } else {
            System.out.println("no dangling Redis process found.");
        }


        System.out.println("Unarchiving distribution tarball...");

        // TODO: untar_grakn_dist

//        local untar_status=$?
//        if [[ $untar_status -ne 0 ]]; then
        System.out.println("Unable to unarchive distribution tarball. Halting test.");
//        force_kill
//        exit 1
//        fi

        System.out.println("Initiating Grakn start/stop test (check if grakn can start and stop properly)");

//        must_properly_start
//        local startup_status=$?
//        if [[ $startup_status -ne 0 ]]; then
        System.out.println("Unable to start Grakn. Halting test.");
//        force_kill
//        exit 1
//        fi
...
//        if [[ $check_pid_exist_status -ne 0 ]]; then
        System.out.println( "Some PID files are missing. Halting test.");
//        force_kill
//        exit 1
//        fi

        System.out.println("Grakn successfully started. Testing data loading and querying...");
//        load_data
//        local load_data_status=$?
//        if [[ $load_data_status -ne 0 ]]; then
        System.out.println( "Data could not be properly loaded. Halting test.");
//        force_kill
//        exit 1
//        fi

        System.out.println("Data was properly loaded. Testing person query...");
//        count_person_equal_to_4
//        local count_person_status=$?
//        if [[ $count_person_status -ne 0 ]]; then
        System.out.println("Person count != 4. Halting test.");
//        force_kill
//        exit 1
//        fi

        System.out.println("Testing marriage query...");
//        count_marriage_equal_to_1
//        local count_marriage_status=$?
//        if [[ $count_marriage_status -ne 0 ]]; then
        System.out.println("Marriage count != 1. Halting test.");
//        force_kill
//        exit 1
//        fi

        System.out.println("Now testing graceful stop...");
//        must_properly_stop
//        local stop_status=$?
...
//        if [[ $check_pid_not_exist_status -ne 0 ]]; then
        System.out.println("Some PID files are still present after cleanup. Halting test.");
//        force_kill
//        exit 1
//        fi

        System.out.println("Wiping out keyspaces...");
//        wipe_out_keyspaces

        System.out.println("Grakn start/stop test successful");
//        exit 0
    }
