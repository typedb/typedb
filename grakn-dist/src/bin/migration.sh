...
if [ "$1" == "csv" ]
then
  java -cp ${CONCATCLASSPATH} -Dlogback.configurationFile="${GRAKN_HOME}/conf/main/logback.xml" -Dgrakn.log.file.postprocessing="${GRAKN_HOME}/logs/grakn-postprocessing.log" -Dgrakn.log.file.main="${GRAKN_HOME}/logs/grakn.log" -Dgrakn.dir="${GRAKN_HOME}/bin" ai.grakn.migration.csv.Main ${1+"$@"}
elif [ "$1" == "json" ]
then
  java -cp ${CONCATCLASSPATH} -Dlogback.configurationFile="${GRAKN_HOME}/conf/main/logback.xml" -Dgrakn.log.file.postprocessing="${GRAKN_HOME}/logs/grakn-postprocessing.log" -Dgrakn.log.file.main="${GRAKN_HOME}/logs/grakn.log" -Dgrakn.dir="${GRAKN_HOME}/bin" ai.grakn.migration.json.Main ${1+"$@"}
elif [ "$1" == "owl" ]
then
...
then
  java -cp ${CONCATCLASSPATH} -Dlogback.configurationFile="${GRAKN_HOME}/conf/main/logback.xml" -Dgrakn.log.file.postprocessing="${GRAKN_HOME}/logs/grakn-postprocessing.log" -Dgrakn.log.file.main="${GRAKN_HOME}/logs/grakn.log" -Dgrakn.dir="${GRAKN_HOME}/bin" ai.grakn.migration.sql.Main ${1+"$@"}
else
