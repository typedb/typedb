...
public class InsertQueryIT {
    ...
    public void testKeyCorrectUsage() throws InvalidKBException {
        tx.execute(Graql.define(
                Graql.type("a-new-type").sub("entity").key("a-new-resource-type"),
                Graql.type("a-new-resource-type").sub(Schema.MetaSchema.ATTRIBUTE.getLabel().getValue()).datatype(Query.DataType.STRING)
        ));
    ...
    public void whenInsertingAThingWithTwoKeyResources_Throw() throws InvalidKBException {
        tx.execute(Graql.define(
                Graql.type("a-new-type").sub("entity").key("a-new-attribute-type"),
                Graql.type("a-new-attribute-type").sub(Schema.MetaSchema.ATTRIBUTE.getLabel().getValue()).datatype(Query.DataType.STRING)
        ));
    ...
    public void testKeyUniqueValue() throws InvalidKBException {
        tx.execute(Graql.define(
                Graql.type("a-new-type").sub("entity").key("a-new-resource-type"),
                Graql.type("a-new-resource-type")
    ...
    public void whenInsertingAnInstanceWithALabel_Throw() {
        ...
        tx.execute(Graql.insert(Graql.type("abc").isa("movie")));
    }
    ...
    public void whenInsertingAResourceWithALabel_Throw() {
        ...
        tx.execute(Graql.insert(Graql.type("bobby").val("bob").isa("name")));
    }
    ...
    public void whenInsertingWithAMatch_ProjectMatchResultsOnVariablesInTheInsert() {
        tx.execute(Graql.define(
                Graql.type("maybe-friends").relates("friend").sub("relationship"),
                Graql.type("person").plays("friend")
        ));
    ...
    public void whenSettingTwoTypes_Throw() {
        ...
        Statement varPattern = new Statement(
                new Variable("x"),
                ImmutableSet.of(new IsaProperty(Graql.type("movie")), new IsaProperty(Graql.type("person")))
        );
    ...
    public void whenInsertingASchemaConcept_Throw() {
        ...
        tx.execute(Graql.insert(Graql.type("new-type").sub(type(ENTITY.getLabel().getValue()))));
    }
    ...
    public void whenModifyingASchemaConceptInAnInsertQuery_Throw() {
        ...
        tx.execute(Graql.insert(Graql.type("movie").plays("actor")));
    }
