...
public class UndefineQueryIT {
    ...
    public void whenUndefiningDataType_DoNothing() {
        tx.execute(Graql.undefine(Graql.type("name").datatype(Query.DataType.STRING)));
    ...
    public void whenUndefiningRegexProperty_TheAttributeTypeHasNoRegex() {
        tx.execute(Graql.define(type(NEW_TYPE.getValue()).sub(ATTRIBUTE).datatype(Query.DataType.STRING).like("abc")));

        assertEquals("abc", tx.<AttributeType>getType(NEW_TYPE).regex());

        tx.execute(Graql.undefine(type(NEW_TYPE.getValue()).like("abc")));
    ...
    public void whenUndefiningRegexPropertyWithWrongRegex_DoNothing() {
        ...
        tx.execute(Graql.undefine(type(NEW_TYPE.getValue()).like("xyz")));
    ...
    public void whenUndefiningComplexSchema_TheEntireSchemaIsRemoved() {
        Collection<Statement> schema = ImmutableList.of(
                Graql.type("pokemon").sub(ENTITY).has("pokedex-no").plays("ancestor").plays("descendant"),
                Graql.type("pokedex-no").sub(ATTRIBUTE).datatype(Query.DataType.DOUBLE),
                Graql.type("evolution").sub(RELATIONSHIP).relates("ancestor").relates("descendant"),
                Graql.type("ancestor").sub(ROLE),
                Graql.type("descendant").sub(ROLE)
        );
    ...
    public void whenUndefiningATypeWithInstances_Throw() {
        ...
        tx.execute(Graql.undefine(Graql.type("movie").sub("production")));
    }
    ...
    public void whenUndefiningASuperConcept_Throw() {
        ...
        tx.execute(Graql.undefine(Graql.type("production").sub(ENTITY)));
    }
    ...
    public void whenUndefiningARoleWithPlayers_Throw() {
        ...
        tx.execute(Graql.undefine(Graql.type("actor").sub(ROLE)));
    }
