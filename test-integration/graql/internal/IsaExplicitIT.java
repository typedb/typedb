...
public class IsaExplicitIT {
    ...
    public void whenInsertIsaExplicit_InsertsADirectInstanceOfAType() {
        ...
        assertEquals(1, tx.execute(Graql.<AggregateQuery>parse("match $z isa! superType1; aggregate count;")).get(0).number().intValue());
        assertEquals(2, tx.execute(Graql.<AggregateQuery>parse("match $z isa superType1; aggregate count;")).get(0).number().intValue());
