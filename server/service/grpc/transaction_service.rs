...
pub(crate) struct TransactionService {
    ...
    query_queue: VecDeque<(Uuid, QueueOptions, QueryContext, Arc<TypeQLPipeline>, Option<GivenRowsGrpc>)>,
...
impl TransactionService {
    ...
    async fn cancel_queued_read_queries(&mut self, interrupt: InterruptType) -> ControlFlow<(), ()> {
        ...
        for (req_id, query_options, context, pipeline, given_rows) in
            self.query_queue.drain(0..self.query_queue.len())
        {
            if query_options.is_query() && is_write_pipeline(&pipeline) {
                write_queries.push_back((req_id, query_options, context, pipeline, given_rows));
            } else {
    ...
    async fn cancel_queued_write_queries(&mut self, interrupt: InterruptType) -> ControlFlow<(), ()> {
        ...
        for (req_id, options, context, pipeline, given_rows) in self.query_queue.drain(0..self.query_queue.len()) {
            if options.is_query() && is_write_pipeline(&pipeline) {
                ...
                read_queries.push_back((req_id, options, context, pipeline, given_rows));
            }
        }
    ...
    async fn finish_queued_write_queries(&mut self, interrupt: InterruptType) -> Result<(), Status> {
        ...
        for (req_id, query_queue_options, context, pipeline, given_rows) in requests.into_iter() {
            match (query_queue_options, is_write_pipeline(&pipeline)) {
                (QueueOptions::Query(query_options), true) => {
                    self.run_write_query(req_id, query_options, context, pipeline, given_rows).await;
                    self.finish_running_write_query_no_transmit(interrupt).await?;
                }
                (queue_options, _) => {
                    self.query_queue.push_back((req_id, queue_options, context, pipeline, given_rows));
                }
            }
        }
        Ok(())
    ...
    async fn may_accept_from_queue(&mut self) {
        ...
        while let Some((req_id, queue_options, context, pipeline, given_rows)) = self.query_queue.pop_front() {
            match (queue_options, is_write_pipeline(&pipeline)) {
                (QueueOptions::Analyze, _) => {
                    debug_assert!(given_rows.is_none());
                    self.run_analyse_query(req_id, context, pipeline).await;
                }
                (QueueOptions::Query(query_options), true) => {
                    self.run_write_query(req_id, query_options, context, pipeline, given_rows).await;
                    return;
                ...
                (QueueOptions::Query(query_options), false) => {
                    self.run_and_activate_read_transmitter(req_id, query_options, context, pipeline, given_rows);
                }
            }
        }
    }
    ...
    async fn handle_analyse_query(
        ...
        let context = QueryContext::instrumented(analyse_req.query);
        let (context, pipeline) = match self.query_manager.as_ref().expect("transaction is open").parse(context) {
            Ok(ParsedQuery::Pipeline(context, pipeline)) => (context, pipeline),
        ...
        if !self.query_queue.is_empty() || self.running_write_query.is_some() {
            self.query_queue.push_back((req_id, QueueOptions::Analyze, context, pipeline, None));
    ...
    async fn handle_query(
        ...
        let context = QueryContext::instrumented(query_req.query);
        let (context, pipeline) = match self.query_manager.as_ref().expect("transaction is open").parse(context) {
            Ok(ParsedQuery::Pipeline(context, pipeline)) => (context, pipeline),
            Ok(ParsedQuery::Schema(context, schema_query)) => {
                ...
                let response = self.handle_query_schema(context, schema_query).await?;
                return Ok(Self::respond_query_response(&self.response_sender, req_id, response).await);
            }
            Err(typedb_source) => {
                let response =
        ...
        if !self.query_queue.is_empty() || self.running_write_query.is_some() {
            self.query_queue.push_back((req_id, QueueOptions::Query(query_options), context, pipeline, given_rows));
            // queued queries are not handled yet so there will be no query response yet
            Ok(Continue(()))
            ...
            match transaction {
                Transaction::Schema(schema_transaction) => {
                    ...
                    let (transaction, result) =
                        spawn_blocking(move || execute_schema_query(schema_transaction, context, query))
                            .await
    ...
    async fn run_analyse_query(&mut self, req_id: Uuid, context: QueryContext, pipeline: Arc<TypeQLPipeline>) {
    ...
    async fn run_write_query(
        ...
        let handle = match self.spawn_blocking_execute_write_query(query_options, context, pipeline, given_rows) {
            Ok(handle) => {
    ...
    fn run_and_activate_read_transmitter(
        ...
        let (sender, receiver) = channel(prefetch_size);
        let worker_handle = self.blocking_read_query_worker(sender, query_options, context, pipeline, given_rows);
        let stream_transmitter = QueryStreamTransmitter::start_new(
            self.response_sender.clone(),
            receiver,
            req_id,
