...
import kb.reasoner.ResolutionIterator;
...
public class ResolutionIterator extends ReasonerQueryIterator {
    ...
    private final Stack<grakn.core.graql.reasoner.state.ResolutionStateImpl> states = new Stack<>();

    private ConceptMap nextAnswer = null;

    private static final Logger LOG = LoggerFactory.getLogger(ResolutionIterator.class);

    public ResolutionIteratorImpl(ResolvableQuery q, Set<ReasonerAtomicQuery> subGoals){
        this.query = q;
    ...
    private ConceptMap findNextAnswer(){
        while(!states.isEmpty()) {
            grakn.core.graql.reasoner.state.ResolutionStateImpl state = states.pop();
            ...
            grakn.core.graql.reasoner.state.ResolutionStateImpl newState = state.generateChildState();
            if (newState != null) {
