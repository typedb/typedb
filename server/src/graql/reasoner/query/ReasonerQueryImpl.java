...
public class ReasonerQueryImpl implements ResolvableQuery {
    ...
    ReasonerQueryImpl(List<grakn.core.graql.reasoner.atom.Atom> atoms, Transaction tx){
        this.tx = tx;
        this.atomSet =  ImmutableSet.<Atomic>builder()
                .addAll(atoms.stream()
    ...
    ReasonerQueryImpl(grakn.core.graql.reasoner.atom.Atom atom) {
    ...
    public boolean isRuleResolvable() {
        return selectAtoms().anyMatch(grakn.core.graql.reasoner.atom.Atom::isRuleResolvable);
    }
    ...
    public boolean isBoundlesslyDisconnected(){
        return !isAtomic()
                ...
                .anyMatch(grakn.core.graql.reasoner.atom.Atom::isDisconnected);
    }

    /**
     * @return true if the query requires direct schema lookups
     */
    public boolean requiresSchema(){ return selectAtoms().anyMatch(grakn.core.graql.reasoner.atom.Atom::requiresSchema);}
     ...
     */
    @Override
    public Stream<grakn.core.graql.reasoner.atom.Atom> selectAtoms() {
        return getAtoms(grakn.core.graql.reasoner.atom.Atom.class).filter(Atomic::isSelectable);
    }

    @Override
    public boolean requiresDecomposition(){
        return this.selectAtoms().anyMatch(grakn.core.graql.reasoner.atom.Atom::requiresDecomposition);
    }
    ...
    public boolean isCacheComplete(){
        ...
        for (ReasonerAtomicQuery query : queries) {
            ...
            Graql.and(
                    ...
                    query.getAtoms(grakn.core.graql.reasoner.atom.Atom.class).collect(Collectors.toList()),
                    Sets.union(
    ...
    public boolean requiresReiteration() {
        ...
        return RuleUtils.subGraphIsCyclical(dependentRules, tx())
                ...
                || selectAtoms().filter(grakn.core.graql.reasoner.atom.Atom::isDisconnected).filter(grakn.core.graql.reasoner.atom.Atom::isRuleResolvable).count() > 1;
    }

    @Override
    public grakn.core.graql.reasoner.state.ResolutionStateImpl resolutionState(ConceptMap sub, Unifier u, grakn.core.graql.reasoner.state.AnswerPropagatorStateImpl parent, Set<ReasonerAtomicQuery> subGoals){
     ...
     */
    public Stream<grakn.core.graql.reasoner.state.ResolutionStateImpl> expandedStates(ConceptMap sub, Unifier u, grakn.core.graql.reasoner.state.AnswerPropagatorStateImpl parent, Set<ReasonerAtomicQuery> subGoals){
    ...
    @Override
    public Iterator<grakn.core.graql.reasoner.state.ResolutionStateImpl> innerStateIterator(grakn.core.graql.reasoner.state.AnswerPropagatorStateImpl parent, Set<ReasonerAtomicQuery> subGoals){
        Iterator<AnswerState> dbIterator;
        Iterator<grakn.core.graql.reasoner.state.AnswerPropagatorStateImpl> subGoalIterator;
