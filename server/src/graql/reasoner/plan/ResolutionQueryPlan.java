...
public class ResolutionQueryPlan {
    ...
    private static ImmutableList<ReasonerQueryImpl> queryPlan(ReasonerQueryImpl query){
        ...
        ImmutableList<grakn.core.graql.reasoner.atom.Atom> plan = resolutionPlan.plan();
        Transaction tx = query.tx();
        LinkedList<grakn.core.graql.reasoner.atom.Atom> atoms = new LinkedList<>(plan);
        List<ReasonerQueryImpl> queries = new LinkedList<>();

        List<grakn.core.graql.reasoner.atom.Atom> nonResolvableAtoms = new ArrayList<>();
        while (!atoms.isEmpty()) {
            grakn.core.graql.reasoner.atom.Atom top = atoms.remove();
