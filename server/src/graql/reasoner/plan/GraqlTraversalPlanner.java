...
public class GraqlTraversalPlanner {
     ...
     */
    public static ImmutableList<grakn.core.graql.reasoner.atom.Atom> plan(ReasonerQueryImpl query) {
        return ImmutableList.copyOf(refinedPlan(query));
    }

    private static long atomPredicates(grakn.core.graql.reasoner.atom.Atom at, Set<IdPredicate> subs){
    ...
    private static List<grakn.core.graql.reasoner.atom.Atom> optimiseCandidates(List<grakn.core.graql.reasoner.atom.Atom> candidates, Set<IdPredicate> subs, Set<Variable> vars){
    ...
    private static List<grakn.core.graql.reasoner.atom.Atom> getCandidates(List<grakn.core.graql.reasoner.atom.Atom> atoms, Set<IdPredicate> subs, Set<Variable> vars){
        List<grakn.core.graql.reasoner.atom.Atom> preCandidates = optimiseCandidates(atoms, subs, vars);
     ...
     */
    private static List<grakn.core.graql.reasoner.atom.Atom> refinedPlan(ReasonerQueryImpl query){
        List<grakn.core.graql.reasoner.atom.Atom> atomsToProcess = query.getAtoms(grakn.core.graql.reasoner.atom.Atom.class).filter(Atomic::isSelectable).collect(Collectors.toList());
        Set<IdPredicate> subs = query.getAtoms(IdPredicate.class).collect(Collectors.toSet());
        Set<Variable> vars = atomsToProcess.stream().anyMatch(grakn.core.graql.reasoner.atom.Atom::isDisconnected)? query.getVarNames() : new HashSet<>();
        List<grakn.core.graql.reasoner.atom.Atom> orderedAtoms = new ArrayList<>();

        while(!atomsToProcess.isEmpty()){
            List<grakn.core.graql.reasoner.atom.Atom> candidates = getCandidates(atomsToProcess, subs, vars);
            ImmutableList<grakn.core.graql.reasoner.atom.Atom> initialPlan = planFromTraversal(atomsToProcess, atomsToPattern(atomsToProcess, subs), query.tx());

            if (candidates.contains(initialPlan.get(0)) || candidates.isEmpty()) {
                orderedAtoms.addAll(initialPlan);
                break;
            } else {
                grakn.core.graql.reasoner.atom.Atom first = candidates.stream().findFirst().orElse(null);
     ...
     */
    public static Conjunction<Pattern> atomsToPattern(List<grakn.core.graql.reasoner.atom.Atom> atoms, Set<IdPredicate> subs){
        return Graql.and(
                Stream.concat(
     ...
     * @return an optimally ordered list of provided atoms
     */
    private static ImmutableList<grakn.core.graql.reasoner.atom.Atom> planFromTraversal(List<grakn.core.graql.reasoner.atom.Atom> atoms, Conjunction<?> queryPattern, Transaction tx){
        Multimap<VarProperty, grakn.core.graql.reasoner.atom.Atom> propertyMap = HashMultimap.create();
        atoms.stream()
        ...
        List<grakn.core.graql.reasoner.atom.Atom> atomList = new ArrayList<>();
