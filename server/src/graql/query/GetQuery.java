...
public class GetQuery implements Query {
    ...
    // TODO: replace with LinkedHashSet<Variable>
    public GetQuery(MatchClause match, Set<Variable> vars) {
        if (vars == null) {
            throw new NullPointerException("Null vars");
        }
        this.vars = Collections.unmodifiableSet(new LinkedHashSet<>(vars));
        if (match == null) {
