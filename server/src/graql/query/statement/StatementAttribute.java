...
public class StatementAttribute extends StatementInstance {
    ...
    public static grakn.core.graql.query.statement.StatementAttribute create(Statement statement, VarProperty varProperty) {
        if (statement instanceof grakn.core.graql.query.statement.StatementAttribute) {
            return ((grakn.core.graql.query.statement.StatementAttribute) statement).addProperty(varProperty);

        } else if (!(statement instanceof StatementRelation)
                && !(statement instanceof StatementType)) {
            return new grakn.core.graql.query.statement.StatementAttribute(statement).addProperty(varProperty);
    ...
    @CheckReturnValue
    private grakn.core.graql.query.statement.StatementAttribute addProperty(VarProperty property) {
        validateNoConflictOrThrow(property);
        LinkedHashSet<VarProperty> newProperties = new LinkedHashSet<>(this.properties());
        newProperties.add(property);
        return new grakn.core.graql.query.statement.StatementAttribute(this.var(), newProperties);
    }
}
