...
public class StatementRelation extends StatementInstance {
    ...
    private static grakn.core.graql.query.statement.StatementRelation createOrCast(Statement statement) {
        if (statement instanceof grakn.core.graql.query.statement.StatementRelation) {
            return (grakn.core.graql.query.statement.StatementRelation) statement;

        } else if (!(statement instanceof StatementAttribute)
                && !(statement instanceof StatementType)) {
            return new grakn.core.graql.query.statement.StatementRelation(statement);
    ...
    public static grakn.core.graql.query.statement.StatementRelation create(Statement statement, VarProperty varProperty) {
        grakn.core.graql.query.statement.StatementRelation relation = createOrCast(statement);
    ...
    public static grakn.core.graql.query.statement.StatementRelation create(Statement statement, RelationProperty.RolePlayer rolePlayer) {
        grakn.core.graql.query.statement.StatementRelation relation = createOrCast(statement);
    ...
    private grakn.core.graql.query.statement.StatementRelation addRolePlayer(RelationProperty.RolePlayer rolePlayer) {
        ...
        if (oldRelationProperty.isPresent()) {
            grakn.core.graql.query.statement.StatementRelation statement = removeProperty(oldRelationProperty.get());
    ...
    @CheckReturnValue
    private grakn.core.graql.query.statement.StatementRelation addProperty(VarProperty property) {
        validateNoConflictOrThrow(property);
        LinkedHashSet<VarProperty> newProperties = new LinkedHashSet<>(this.properties());
        newProperties.add(property);
        return new grakn.core.graql.query.statement.StatementRelation(this.var(), newProperties);
    }

    private grakn.core.graql.query.statement.StatementRelation removeProperty(VarProperty property) {
        Variable name = var();
        LinkedHashSet<VarProperty> newProperties = new LinkedHashSet<>(this.properties());
        newProperties.remove(property);
        return new grakn.core.graql.query.statement.StatementRelation(name, newProperties);
    }
}
