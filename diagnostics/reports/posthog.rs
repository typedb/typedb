...
impl PosthogPayloadBuilder {
    fn new(diagnostics: &Diagnostics, api_key: String) -> Self {
        let typedb = diagnostics.typedb();
        let server_properties: PosthogServerPropertiesReport = typedb.server_properties.to_state_report().into();
        let server_metrics_full: PosthogServerReport = typedb.server_metrics.to_full_state_report().into();
        let server_metrics_minimal: PosthogServerReport = typedb.server_metrics.to_minimal_state_report().into();
        Self {
            api_key,
...
pub(crate) fn to_full_posthog_reporting_json(diagnostics: &Diagnostics, api_key: &str) -> Value {
    ...
    for metrics in diagnostics.typedb().lock_load_metrics_read().values() {
    ...
    for client in ALL_CLIENT_ENDPOINTS {
        for metrics in diagnostics.typedb().lock_action_metrics_read(client).values() {
        ...
        for metrics in diagnostics.typedb().lock_error_metrics_read(client).values() {
