...
public class ThingTypeSteps {
    ...
    public static ThingType get_thing_type(final RootLabel rootLabel, final String typeLabel) {
        switch (rootLabel) {
            case ENTITY:
    ...
    public void thing_type_root_get_supertypes_contain(final List<String> superLabels) {
        final Set<String> actuals = tx().concepts().getRootThingType().getSupertypes().map(ThingType::getLabel).collect(toSet());
        assertTrue(actuals.containsAll(superLabels));
    }

    @Then("thing type root get supertypes do not contain:")
    public void thing_type_root_get_supertypes_do_not_contain(final List<String> superLabels) {
        final Set<String> actuals = tx().concepts().getRootThingType().getSupertypes().map(ThingType::getLabel).collect(toSet());
    ...
    public void thing_type_root_get_subtypes_contain(final List<String> subLabels) {
        final Set<String> actuals = tx().concepts().getRootThingType().getSubtypes().map(ThingType::getLabel).collect(toSet());
        assertTrue(actuals.containsAll(subLabels));
    }

    @Then("thing type root get subtypes do not contain:")
    public void thing_type_root_get_subtypes_do_not_contain(final List<String> subLabels) {
        final Set<String> actuals = tx().concepts().getRootThingType().getSubtypes().map(ThingType::getLabel).collect(toSet());
    ...
    public void put_thing_type(final RootLabel rootLabel, final String typeLabel) {
        switch (rootLabel) {
            case ENTITY:
    ...
    public void delete_thing_type(final RootLabel rootLabel, final String typeLabel) {
        get_thing_type(rootLabel, typeLabel).delete();
    }

    @Then("delete {root_label} type: {type_label}; throws exception")
    public void delete_thing_type_throws_exception(final RootLabel rootLabel, final String typeLabel) {
        assertThrows(() -> get_thing_type(rootLabel, typeLabel).delete());
    }

    @Then("{root_label}\\( ?{type_label} ?) is null: {bool}")
    public void thing_type_is_null(final RootLabel rootLabel, final String typeLabel, final boolean isNull) {
        assertEquals(isNull, isNull(get_thing_type(rootLabel, typeLabel)));
    }

    @When("{root_label}\\( ?{type_label} ?) set label: {type_label}")
    public void thing_type_set_label(final RootLabel rootLabel, final String typeLabel, final String newLabel) {
        get_thing_type(rootLabel, typeLabel).setLabel(newLabel);
    }

    @Then("{root_label}\\( ?{type_label} ?) get label: {type_label}")
    public void thing_type_get_label(final RootLabel rootLabel, final String typeLabel, final String getLabel) {
        assertEquals(getLabel, get_thing_type(rootLabel, typeLabel).getLabel());
    }

    @When("{root_label}\\( ?{type_label} ?) set abstract: {bool}")
    public void thing_type_set_abstract(final RootLabel rootLabel, final String typeLabel, final boolean isAbstract) {
    ...
    public void thing_type_is_abstract(final RootLabel rootLabel, final String typeLabel, final boolean isAbstract) {
        assertEquals(isAbstract, get_thing_type(rootLabel, typeLabel).isAbstract());
    }

    @When("{root_label}\\( ?{type_label} ?) set supertype: {type_label}")
    public void thing_type_set_supertype(final RootLabel rootLabel, final String typeLabel, final String superLabel) {
        switch (rootLabel) {
            case ENTITY:
    ...
    public void thing_type_set_supertype_throws_exception(final RootLabel rootLabel, final String typeLabel, final String superLabel) {
        switch (rootLabel) {
            case ENTITY:
                final EntityType entitySuperType = tx().concepts().getEntityType(superLabel);
    ...
    public void thing_type_get_supertype(final RootLabel rootLabel, final String typeLabel, final String superLabel) {
    ...
    public void thing_type_get_supertypes_contain(final RootLabel rootLabel, final String typeLabel, final List<String> superLabels) {
        final Set<String> actuals = get_thing_type(rootLabel, typeLabel).getSupertypes().map(ThingType::getLabel).collect(toSet());
        assertTrue(actuals.containsAll(superLabels));
    }

    @Then("{root_label}\\( ?{type_label} ?) get supertypes do not contain:")
    public void thing_type_get_supertypes_do_not_contain(final RootLabel rootLabel, final String typeLabel, final List<String> superLabels) {
        final Set<String> actuals = get_thing_type(rootLabel, typeLabel).getSupertypes().map(ThingType::getLabel).collect(toSet());
        for (String superLabel : superLabels) {
            assertFalse(actuals.contains(superLabel));
        }
    ...
    public void thing_type_get_subtypes_contain(final RootLabel rootLabel, final String typeLabel, final List<String> subLabels) {
        final Set<String> actuals = get_thing_type(rootLabel, typeLabel).getSubtypes().map(ThingType::getLabel).collect(toSet());
        assertTrue(actuals.containsAll(subLabels));
    }

    @Then("{root_label}\\( ?{type_label} ?) get subtypes do not contain:")
    public void thing_type_get_subtypes_do_not_contain(final RootLabel rootLabel, final String typeLabel, final List<String> subLabels) {
        final Set<String> actuals = get_thing_type(rootLabel, typeLabel).getSubtypes().map(ThingType::getLabel).collect(toSet());
        for (String subLabel : subLabels) {
            assertFalse(actuals.contains(subLabel));
        }
    ...
    public void thing_type_set_owns_key_type(final RootLabel rootLabel, final String typeLabel, final String attTypeLabel) {
    ...
    public void thing_type_set_owns_key_type_as(final RootLabel rootLabel, final String typeLabel, final String attTypeLabel, final String overriddenLabel) {
        final AttributeType attributeType = tx().concepts().getAttributeType(attTypeLabel);
    ...
    public void thing_type_set_owns_key_type_throws_exception(final RootLabel rootLabel, final String typeLabel, final String attributeLabel) {
    ...
    public void thing_type_set_owns_key_type_as_throws_exception(final RootLabel rootLabel, final String typeLabel, final String attributeLabel, final String overriddenLabel) {
        final AttributeType attributeType = tx().concepts().getAttributeType(attributeLabel);
        final AttributeType overriddenType = tx().concepts().getAttributeType(overriddenLabel);
    ...
    public void thing_type_unset_owns_key_type(final RootLabel rootLabel, final String typeLabel, final String attributeLabel) {
        final AttributeType attributeType = tx().concepts().getAttributeType(attributeLabel);
        get_thing_type(rootLabel, typeLabel).unsetOwns(attributeType);
    }

    @Then("{root_label}\\( ?{type_label} ?) get owns key types contain:")
    public void thing_type_get_owns_key_types_contain(final RootLabel rootLabel, final String typeLabel, final List<String> attributeLabels) {
        final Set<String> actuals = get_thing_type(rootLabel, typeLabel).getOwns(true).map(Type::getLabel).collect(toSet());
        assertTrue(actuals.containsAll(attributeLabels));
    }

    @Then("{root_label}\\( ?{type_label} ?) get owns key types do not contain:")
    public void thing_type_get_owns_key_types_do_not_contain(final RootLabel rootLabel, final String typeLabel, final List<String> attributeLabels) {
        final Set<String> actuals = get_thing_type(rootLabel, typeLabel).getOwns(true).map(Type::getLabel).collect(toSet());
    ...
    public void thing_type_set_owns_attribute_type(final RootLabel rootLabel, final String typeLabel, final String attributeLabel) {
        final AttributeType attributeType = tx().concepts().getAttributeType(attributeLabel);
    ...
    public void thing_type_set_owns_attribute_throws_exception(final RootLabel rootLabel, final String typeLabel, final String attributeLabel) {
        final AttributeType attributeType = tx().concepts().getAttributeType(attributeLabel);
    ...
    public void thing_type_set_owns_attribute_type_as(final RootLabel rootLabel, final String typeLabel, final String attributeLabel, final String overriddenLabel) {
        final AttributeType attributeType = tx().concepts().getAttributeType(attributeLabel);
        final AttributeType overriddenType = tx().concepts().getAttributeType(overriddenLabel);
    ...
    public void thing_type_set_owns_attribute_as_throws_exception(final RootLabel rootLabel, final String typeLabel, final String attributeLabel, final String overriddenLabel) {
        final AttributeType attributeType = tx().concepts().getAttributeType(attributeLabel);
        final AttributeType overriddenType = tx().concepts().getAttributeType(overriddenLabel);
    ...
    public void thing_type_unset_owns_attribute_type(final RootLabel rootLabel, final String typeLabel, final String attributeLabel) {
        final AttributeType attributeType = tx().concepts().getAttributeType(attributeLabel);
        get_thing_type(rootLabel, typeLabel).unsetOwns(attributeType);
    }
    ...
    public void thing_type_get_owns_attribute_types_do_not_contain(final RootLabel rootLabel, final String typeLabel, final List<String> attributeLabels) {
        final Set<String> actuals = get_thing_type(rootLabel, typeLabel).getOwns().map(Type::getLabel).collect(toSet());
        for (String attributeLabel : attributeLabels) {
            assertFalse(actuals.contains(attributeLabel));
        }
    ...
    public void thing_type_set_plays_role(final RootLabel rootLabel, final String typeLabel, final Parameters.ScopedLabel roleLabel) {
    ...
    public void thing_type_set_plays_role_throws_exception(final RootLabel rootLabel, final String typeLabel, final Parameters.ScopedLabel roleLabel) {
        final RoleType roleType = tx().concepts().getRelationType(roleLabel.scope()).getRelates(roleLabel.role());
    ...
    public void thing_type_set_plays_role_as(final RootLabel rootLabel, final String typeLabel, final Parameters.ScopedLabel roleLabel, final String overriddenLabel) {
        final RoleType roleType = tx().concepts().getRelationType(roleLabel.scope()).getRelates(roleLabel.role());
    ...
    public void thing_type_set_plays_role_as_throws_exception(final RootLabel rootLabel, final String typeLabel, final Parameters.ScopedLabel roleLabel, final Parameters.ScopedLabel overriddenLabel) {
        final RoleType roleType = tx().concepts().getRelationType(roleLabel.scope()).getRelates(roleLabel.role());
        final RoleType overriddenType = tx().concepts().getRelationType(overriddenLabel.scope()).getRelates(overriddenLabel.role());
    ...
    public void thing_type_get_playing_roles_contain(final RootLabel rootLabel, final String typeLabel, final List<Parameters.ScopedLabel> roleLabels) {
    ...
    public void thing_type_get_playing_roles_do_not_contain(final RootLabel rootLabel, final String typeLabel, final List<Parameters.ScopedLabel> roleLabels) {
        final Set<Parameters.ScopedLabel> actuals = get_thing_type(rootLabel, typeLabel).getPlays().map(
                r -> new Parameters.ScopedLabel(r.getScope(), r.getLabel())
        ).collect(toSet());
