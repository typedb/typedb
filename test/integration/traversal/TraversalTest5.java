...
public class TraversalTest5 {
        ...
        try (RocksSession session = grakn.session(database, DATA)) {
            try (RocksTransaction transaction = session.transaction(WRITE)) {
                String insertStr = "insert\n" +
                        "$x isa person, has age 30;\n" +
                        "$y isa person, has age 30;\n" +
                        "$z isa person, has age 60;\n";
    ...
    public void test_mean() {
        ...
        try (RocksTransaction transaction = session.transaction(READ)) {
            ResourceIterator<ConceptMap> answers = transaction.query().match(parseQuery("match $x isa person, has age $a;").asMatch(), false);
            assertEquals(3, answers.count());
            Numeric answer = transaction.query().match(parseQuery("match $x isa person, has age $a; mean $a;").asMatchAggregate(), false);
    ...
    public void test_relation() {
        System.out.println();
        try (RocksTransaction transaction = session.transaction(READ)) {
            ResourceIterator<ConceptMap> answers = transaction.query().match(parseQuery("match (friend: $x, friend: $x);").asMatch(), false);
...
//    }
    ...
    public void test_traversal() {
        try (RocksTransaction transaction = session.transaction(READ)) {
            ...
            answers = transaction.query().match(parseQuery("match $m (wife: $x, husband: $y) isa hetero-marriage;").asMatch(), false);
            assertTrue(answers.hasNext());
            ...
            try {
                transaction.query().match(parseQuery("match $m (wife: $x, husband: $y) isa civil-marriage;").asMatch(), false).toList();
                fail();
            } catch (Throwable e) {
                assertTrue(true);
            }

            answers = transaction.query().match(parseQuery("match $m (wife: $x, husband: $y) isa marriage;").asMatch(), false);
            assertTrue(answers.hasNext());
            answerList = answers.toList();
            printAnswer(name, answerList);
            assertEquals(1, answerList.size());

            answers = transaction.query().match(parseQuery("match $m (wife: $x, husband: $y) isa relation;").asMatch(), false);
            assertTrue(answers.hasNext());
            answerList = answers.toList();
            printAnswer(name, answerList);
            assertEquals(1, answerList.size());

            answers = transaction.query().match(parseQuery("match $m (spouse: $x, spouse: $y) isa hetero-marriage;").asMatch(), false);
            assertTrue(answers.hasNext());
            answerList = answers.toList();
            printAnswer(name, answerList);
            assertEquals(2, answerList.size());

            answers = transaction.query().match(parseQuery("match $m (spouse: $x, spouse: $y) isa civil-marriage;").asMatch(), false);
            assertTrue(answers.hasNext());
            answerList = answers.toList();
            printAnswer(name, answerList);
            assertEquals(2, answerList.size());

            answers = transaction.query().match(parseQuery("match $m (spouse: $x, spouse: $y) isa marriage;").asMatch(), false);
            assertTrue(answers.hasNext());
            answerList = answers.toList();
            printAnswer(name, answerList);
            assertEquals(4, answerList.size());

            answers = transaction.query().match(parseQuery("match $m (spouse: $x, spouse: $y) isa relation;").asMatch(), false);
            assertTrue(answers.hasNext());
            answerList = answers.toList();
            printAnswer(name, answerList);
            assertEquals(4, answerList.size());

            answers = transaction.query().match(parseQuery("match $m (role: $x, role: $y) isa hetero-marriage;").asMatch(), false);
            assertTrue(answers.hasNext());
            answerList = answers.toList();
            printAnswer(name, answerList);
            assertEquals(2, answerList.size());

            answers = transaction.query().match(parseQuery("match $m (role: $x, role: $y) isa civil-marriage;").asMatch(), false);
            assertTrue(answers.hasNext());
            answerList = answers.toList();
            printAnswer(name, answerList);
            assertEquals(2, answerList.size());

            answers = transaction.query().match(parseQuery("match $m (role: $x, role: $y) isa marriage;").asMatch(), false);
            assertTrue(answers.hasNext());
            answerList = answers.toList();
            printAnswer(name, answerList);
            assertEquals(4, answerList.size());

            answers = transaction.query().match(parseQuery("match $m (role: $x, role: $y) isa relation;").asMatch(), false);
            assertTrue(answers.hasNext());
            answerList = answers.toList();
            printAnswer(name, answerList);
            assertEquals(4, answerList.size());
    ...
    public void printAnswer(AttributeType.String name, List<ConceptMap> answers) {
        for (ConceptMap answer : answers) {
            ...
            System.out.println("m: " + answer.get("m").asThing().getHas(name).findFirst().get().getValue());
            System.out.println("-------------");
        }
    }
