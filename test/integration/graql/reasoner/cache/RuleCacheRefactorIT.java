...
public class RuleCacheRefactorIT {
    ...
    public static void loadContext() {
        genericSchemaSession = SessionUtil.serverlessSessionWithNewKeyspace(storage.createCompatibleServerConfig());
    ...
    public void whenGettingRulesWithType_correctRulesAreObtained(){
        try(Transaction tx = genericSchemaSession.transaction(Transaction.Type.WRITE)) {
            ...
            rulesWithTransitive.stream()
                    .map(ruleCache::getRule)
                    .forEach(r -> assertEquals(rulesWithTransitive, r.getHead().getAtom().getSchemaConcept()));
        }
    }
