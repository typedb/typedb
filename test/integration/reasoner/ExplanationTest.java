...
public class ExplanationTest {
    ...
    public void test_relation_explainable() {
        ...
        try (CoreSession session = databaseMgr.session(database, Arguments.Session.Type.DATA)) {
            ...
            try (CoreTransaction txn = singleThreadElgTransaction(session, Arguments.Transaction.Type.READ, (new Options.Transaction().explain(true)))) {
                List<? extends ConceptMap> ans = txn.query().match(TypeQL.parseQuery("match (friend: $p1, friend: $p2) isa friendship; $p1 has name $na;").asGet()).toList();
    ...
    public void test_has_variable_explainable_two_ways() {
        ...
        try (CoreSession session = databaseMgr.session(database, Arguments.Session.Type.DATA)) {
            ...
            try (CoreTransaction txn = singleThreadElgTransaction(session, Arguments.Transaction.Type.READ, (new Options.Transaction().explain(true)))) {
                List<? extends ConceptMap> ans = txn.query().match(TypeQL.parseQuery("match $x isa user, has permission \"write\";").asGet()).toList();
    ...
    public void test_all_transitive_explanations() {
        ...
        try (CoreSession session = databaseMgr.session(database, Arguments.Session.Type.DATA)) {
            ...
            try (CoreTransaction txn = singleThreadElgTransaction(session, Arguments.Transaction.Type.READ, (new Options.Transaction().explain(true)))) {
                List<? extends ConceptMap> ans = txn.query().match(TypeQL.parseQuery("match $r isa location-hierarchy;").asGet()).toList();
    ...
    public void test_nested_explanations() {
        ...
        try (CoreSession session = databaseMgr.session(database, Arguments.Session.Type.DATA)) {
            ...
            try (CoreTransaction txn = singleThreadElgTransaction(session, Arguments.Transaction.Type.READ, (new Options.Transaction().explain(true)))) {
                List<? extends ConceptMap> ans = txn.query().match(TypeQL.parseQuery("match ($x) isa friendship;").asGet()).toList();
                assertEquals(2, ans.size());
