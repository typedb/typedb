...
public class ExplanationTest {
    ...
    private TransactionImpl singleThreadElgTransaction(SessionImpl session, Arguments.Transaction.Type transactionType) {
        return singleThreadElgTransaction(session, transactionType, new Options.Transaction().infer(true));
    }

    private TransactionImpl singleThreadElgTransaction(SessionImpl session, Arguments.Transaction.Type transactionType, Options.Transaction options) {
        TransactionImpl transaction = session.transaction(transactionType, options.infer(true));
    ...
    public void test_disjunction_explainable() {
        try (SessionImpl session = databaseManager.session(database, Arguments.Session.Type.SCHEMA)) {
            try (TransactionImpl txn = singleThreadElgTransaction(session, Arguments.Transaction.Type.WRITE)) {
        ...
        try (SessionImpl session = databaseManager.session(database, Arguments.Session.Type.DATA)) {
            ...
            try (TransactionImpl txn = singleThreadElgTransaction(session, Arguments.Transaction.Type.READ, (new Options.Transaction().explain(true)))) {
    ...
    private List<Explanation> assertSingleExplainableExplanations(ConceptMap ans, int anonymousConcepts, int explainablesCount,
                                                                  int explanationsCount, TransactionImpl txn) {
