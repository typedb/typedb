...
public class ResolutionTest {
                ...
                for (int i = 0; i < answerCount; i++) {
                    root.tell(actor ->
                                      actor.receiveRequest(
                                              new Request(new Request.Path(root), DownstreamVars.Initial.empty(), null),
                                              0)
                    );
    ...
    private void assertResponses(final Actor<RootResolver> root, final LinkedBlockingQueue<ResolutionAnswer> responses,
        ...
        for (int i = 0; i < n; i++) {
            root.tell(actor ->
                              actor.receiveRequest(
                                      new Request(new Request.Path(root), DownstreamVars.Initial.empty(), ResolutionAnswer.Derivation.EMPTY),
                                      0)
            );
        }
