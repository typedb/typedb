...
public class MigratorTest {
    ...
    public void test_import_export_schema() throws IOException {
        ...
        try (TypeDB.DatabaseManager typedbDatabaseMgr = RocksDatabaseManager.open(options)) {
            typedbDatabaseMgr.create(database);
            String savedSchema = new String(Files.readAllBytes(schemaPath), UTF_8);
            runSchema(typedbDatabaseMgr, savedSchema);
            String exportedSchema = typedbDatabaseMgr.get(database).schema();
    ...
    public void test_import_export_data() throws IOException {
        Util.resetDirectory(dataDir);
        try (RocksDatabaseManager typedbDatabaseMgr = RocksDatabaseManager.open(options)) {
            typedbDatabaseMgr.create(database);
            String schema = new String(Files.readAllBytes(schemaPath), UTF_8);
            runSchema(typedbDatabaseMgr, schema);
            new DataImporter(typedbDatabaseMgr, database, dataPath, Version.VERSION).run();
            new DataExporter(typedbDatabaseMgr, database, exportDataPath, Version.VERSION).run();
            assertEquals(getChecksums(dataPath), getChecksums(exportDataPath));
        }
    }

    private void runSchema(TypeDB.DatabaseManager typedbDatabaseMgr, String schema) {
        try (TypeDB.Session session = typedbDatabaseMgr.session(database, Arguments.Session.Type.SCHEMA)) {
