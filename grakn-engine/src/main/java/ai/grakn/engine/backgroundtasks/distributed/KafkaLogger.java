...
public class KafkaLogger {
    ...
    public void debug(String msg) {
        if(logLevel.level() <= LogLevel.DEBUG.level())
            sendMsg(LogLevel.DEBUG.toString(), Thread.currentThread().getStackTrace()[1].toString(), msg);
        LOG.debug(msg);
    }

    public void info(String msg) {
        if(logLevel.level() <= LogLevel.INFO.level())
            sendMsg(LogLevel.INFO.toString(), Thread.currentThread().getStackTrace()[1].toString(), msg);
        LOG.info(msg);
    }

    public void warn(String msg) {
        if(logLevel.level() <= LogLevel.WARN.level())
        sendMsg(LogLevel.WARN.toString(), Thread.currentThread().getStackTrace()[1].toString(), msg);
        LOG.warn(msg);
    }

    public void error(String msg) {
        if(logLevel.level() <= LogLevel.ERROR.level())
        sendMsg(LogLevel.ERROR.toString(), Thread.currentThread().getStackTrace()[1].toString(), msg);
        LOG.error(msg);
    }
