...
import static ai.grakn.engine.controller.Utilities.getEntityTypes;
import static ai.grakn.engine.controller.Utilities.getKeyspace;
import static ai.grakn.engine.controller.Utilities.getRelationTypes;
import static ai.grakn.engine.controller.Utilities.getRoleTypes;
...
public class VisualiserController {
    ...
    public VisualiserController(Service spark) {
        ...
        spark.get(REST.WebPath.CONCEPT_TYPES_URI + ID_PARAMETER, this::conceptRelatedTypes);
    }
    ...
    private String conceptById(Request req, Response res) {
        ...
        try (GraknGraph graph = getInstance().getGraph(keyspace, GraknTxType.READ)) {
    ...
    private String conceptRelatedTypes(Request req, Response res) {
        String keyspace = getKeyspace(req);
        ...
        try (GraknGraph graph = getInstance().getGraph(keyspace, GraknTxType.READ)) {
            ...
            if (concept.isEntity()) {
                final Set<RoleType> roleTypesPlayerByConcept = concept.asEntity().type().playsRoles().stream().collect(Collectors.toSet());

                Json typesStructure = Json.object();

                typesStructure.set("roles", getRoleTypes(roleTypesPlayerByConcept, concept, limit, keyspace));
                typesStructure.set("relations", getRelationTypes(roleTypesPlayerByConcept, concept, limit, keyspace));
                typesStructure.set("entities", getEntityTypes(roleTypesPlayerByConcept, concept, limit, keyspace));

                return typesStructure.toString();
            } else {
                return "{}";
            }
        } catch (Exception e) {
    ...
    private String explain(Request req, Response res) {
        ...
        try (GraknGraph graph = getInstance().getGraph(keyspace, GraknTxType.READ)) {
            ...
            explanationAnswersToHAL(((MatchQuery) parsedQuery).admin().streamWithAnswers(), keyspace, limit).asList().forEach(conceptsArray::add);
    ...
    private String compute(Request req, Response res) {
        try (GraknGraph graph = getInstance().getGraph(getKeyspace(req), GraknTxType.READ)) {
    ...
    private String formatAsHAL(MatchQuery query, String keyspace, int limit) {
        Collection<Answer> results = query.admin().streamWithAnswers().collect(toList());
