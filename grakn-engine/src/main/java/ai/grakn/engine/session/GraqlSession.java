...
class GraqlSession {
    ...
    void receiveQuery(Json json) {
        queryExecutor.submit(() -> {
            String queryString = json.at(REST.RemoteShell.QUERY).asString();
    ...
    void autocomplete(Json json) {
        queryExecutor.submit(() -> {
            String queryString = json.at(REST.RemoteShell.QUERY).asString();
            int cursor = json.at(REST.RemoteShell.AUTOCOMPLETE_CURSOR).asInteger();
    ...
    private void sendQueryResult(String result) {
        ...
        for (String resultChunk : splitResult) {
            sendJson(Json.object(
                    ...
                    REST.RemoteShell.QUERY_RESULT, resultChunk
            ));
        }
    }
    ...
    private void sendQueryError(String errorMessage) {
        ...
        for (String errorChunk : splitError) {
            sendJson(Json.object(
                    REST.RemoteShell.ACTION, REST.RemoteShell.ACTION_ERROR,
                    REST.RemoteShell.ERROR, errorChunk
            ));
        }
    ...
    private void sendCommitError(String errorMessage) {
        sendJson(Json.object(
                REST.RemoteShell.ACTION, REST.RemoteShell.ACTION_COMMIT,
                REST.RemoteShell.ERROR, errorMessage
        ));
    }
    ...
    private void sendAutocomplete(Autocomplete autocomplete) {
        sendJson(Json.object(
                ...
                REST.RemoteShell.AUTOCOMPLETE_CANDIDATES, autocomplete.getCandidates(),
                REST.RemoteShell.AUTOCOMPLETE_CURSOR, autocomplete.getCursorPosition()
        ));
    }
